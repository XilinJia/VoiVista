package ac.mdiq.vista.player.notification
...
object NotificationConstants {
    ...
    fun getActionName(context: Context, @Action action: Int): String {
        return when (action) {
            ...
            Action.SMART_REWIND_PREVIOUS -> concatenateStrings(context.getString(R.string.exo_controls_rewind_description), context.getString(R.string.exo_controls_previous_description))
            Action.SMART_FORWARD_NEXT -> concatenateStrings(context.getString(R.string.exo_controls_fastforward_description), context.getString(R.string.exo_controls_next_description))
            Action.PLAY_PAUSE -> concatenateStrings(context.getString(R.string.exo_controls_play_description), context.getString(R.string.exo_controls_pause_description))
            Action.PLAY_PAUSE_BUFFERING -> concatenateStrings(context.getString(R.string.exo_controls_play_description), context.getString(R.string.exo_controls_pause_description), context.getString(R.string.notification_action_buffering))
     ...
     */
    fun getCompactSlotsFromPreferences(context: Context, sharedPreferences: SharedPreferences): Collection<Int> {
        val compactSlots: SortedSet<Int> = TreeSet()
        for (i in 0..2) {
            val compactSlot = sharedPreferences.getInt(context.getString(SLOT_COMPACT_PREF_KEYS[i]), Int.MAX_VALUE)
            // settings not yet populated, return default values
            if (compactSlot == Int.MAX_VALUE) return SLOT_COMPACT_DEFAULTS
            // compact slot is < 0 if there are less than 3 checked checkboxes
            if (compactSlot >= 0) compactSlots.add(compactSlot)
        }
        return compactSlots
    }
