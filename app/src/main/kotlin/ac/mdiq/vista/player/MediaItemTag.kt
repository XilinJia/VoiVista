...
interface MediaItemTag {
    ...
    class Quality private constructor(val sortedVideoStreams: List<VideoStream>, val selectedVideoStreamIndex: Int) {
        val selectedVideoStream: VideoStream?
            get() = if (selectedVideoStreamIndex < 0 || selectedVideoStreamIndex >= sortedVideoStreams.size) null else sortedVideoStreams[selectedVideoStreamIndex]
    ...
    class AudioTrack private constructor(val audioStreams: List<AudioStream>, val selectedAudioStreamIndex: Int) {
        val selectedAudioStream: AudioStream?
            get() = if (selectedAudioStreamIndex < 0 || selectedAudioStreamIndex >= audioStreams.size) null else audioStreams[selectedAudioStreamIndex]
