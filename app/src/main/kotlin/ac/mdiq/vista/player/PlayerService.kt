...
import ac.mdiq.vista.ui.player.NotificationPlayerUi
...
//            ?.ifPresent { ui: MediaSessionPlayerUi -> ui.handleMediaButtonIntent(intent) }
    ...
    fun stopForImmediateReusing() {
        ...
        if (playerManager != null && !playerManager!!.exoPlayerIsNull()) playerManager!!.smoothStopForImmediateReusing()
    }
    ...
    class AudioServiceLeakFix internal constructor(base: Context?) : ContextWrapper(base) {
        override fun getSystemService(name: String): Any {
            if (AUDIO_SERVICE == name) return applicationContext.getSystemService(name)
