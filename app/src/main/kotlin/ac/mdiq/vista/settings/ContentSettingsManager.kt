...
import ac.mdiq.vista.giga.io.SharpOutputStream
import ac.mdiq.vista.giga.io.StoredFileHelper
import ac.mdiq.vista.util.Logd
import ac.mdiq.vista.util.ZipHelper
import java.io.IOException
import java.io.ObjectInputStream
import java.io.ObjectOutputStream
import java.util.zip.ZipOutputStream

class ContentSettingsManager(private val fileLocator: VistaFileLocator) {
    companion object {
    ...
    fun exportDatabase(preferences: SharedPreferences, file: StoredFileHelper) {
        ...
        ZipOutputStream(SharpOutputStream(file.stream).buffered())
            .use { outZip ->
                ZipHelper.addFileToZip(outZip, fileLocator.db.path, "vista.db")
                try {
                ...
                ZipHelper.addFileToZip(outZip, fileLocator.settings.path, "voivista.settings")
            }
    }
    ...
    fun extractDb(file: StoredFileHelper): Boolean {
        val success = ZipHelper.extractFileFromZip(file, fileLocator.db.path, "vista.db")
        if (success) {
    ...
    fun extractSettings(file: StoredFileHelper): Boolean {
        return ZipHelper.extractFileFromZip(file, fileLocator.settings.path, "voivista.settings")
    }
