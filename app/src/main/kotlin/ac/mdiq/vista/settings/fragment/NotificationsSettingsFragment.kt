...
import ac.mdiq.vista.local.feed.notifications.NotificationHelper
import ac.mdiq.vista.local.feed.notifications.NotificationWorker
import ac.mdiq.vista.local.feed.notifications.ScheduleOptions
...
class NotificationsSettingsFragment : BasePreferenceFragment(), OnSharedPreferenceChangeListener {
    ...
    override fun onCreatePreferences(savedInstanceState: Bundle?, rootKey: String?) {
        ...
        preferenceScreen.isEnabled = NotificationHelper.areNotificationsEnabledOnDevice(requireContext())
    }
    ...
    override fun onSharedPreferenceChanged(sharedPreferences: SharedPreferences?, key: String?) {
        ...
        if (key == getString(R.string.streams_notifications_interval_key) || key == getString(R.string.streams_notifications_network_key))
            ...
            if (NotificationHelper.areNewStreamsNotificationsEnabled(context)) NotificationWorker.schedule(context)
    ...
    override fun onResume() {
        ...
        if (!enabled) {
            if (notificationWarningSnackbar == null) {
                notificationWarningSnackbar = Snackbar.make(listView, R.string.notifications_disabled, Snackbar.LENGTH_INDEFINITE, ).apply {
                    setAction(R.string.settings) {
                        NotificationHelper.openVoiVistaSystemNotificationSettings(it.context)
                    }
    ...
    private fun onError(e: Throwable) {
        ErrorUtil.showSnackbar(this, ErrorInfo(e, UserAction.SUBSCRIPTION_GET, "Get subscriptions list"))
    }
}
