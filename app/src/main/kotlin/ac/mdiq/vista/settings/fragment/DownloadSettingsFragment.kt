...
class DownloadSettingsFragment : BasePreferenceFragment() {
    ...
    override fun onCreatePreferences(savedInstanceState: Bundle?, rootKey: String?) {
        ...
        prefUseSaf!!.isChecked = VistaSettings.useStorageAccessFramework(requireContext())
        ...
        if (hasInvalidPath(downloadPathVideoPreference!!) || hasInvalidPath(downloadPathAudioPreference!!)) updatePreferencesSummary()

        prefStorageAsk?.onPreferenceChangeListener = Preference.OnPreferenceChangeListener { preference: Preference?, value: Any? ->
    ...
    private fun showPathInSummary(prefKey: String?, @StringRes defaultString: Int, target: Preference?) {
    ...
    override fun onPreferenceTreeClick(preference: Preference): Boolean {
        ...
        when (key) {
            storageUseSafPreference -> {
                if (!VistaSettings.useStorageAccessFramework(requireContext())) {
                    VistaSettings.saveDefaultVideoDownloadDirectory(requireContext())
                    VistaSettings.saveDefaultAudioDownloadDirectory(requireContext())
                } else defaultPreferences.edit().putString(downloadPathVideoPreference, null).putString(downloadPathAudioPreference, null).apply()
                updatePreferencesSummary()
                return true
            }
