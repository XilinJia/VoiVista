...
import ac.mdiq.vista.util.DeviceUtils
import ac.mdiq.vista.util.DeviceUtils.isFireTv
import ac.mdiq.vista.util.DeviceUtils.shouldSupportMediaTunneling
import java.io.File
 ...
 */
object VistaSettings {
    @JvmStatic
    fun initSettings(context: Context) {
        // check if the last used preference version is set
        // to determine whether this is the first app run
        val lastUsedPrefVersion = PreferenceManager.getDefaultSharedPreferences(context).getInt(context.getString(R.string.last_used_preferences_version), -1)
    ...
    private fun saveDefaultDirectory(context: Context, keyID: Int, defaultDirectoryName: String) {
        if (!useStorageAccessFramework(context)) {
            val prefs = PreferenceManager.getDefaultSharedPreferences(context)
            val key = context.getString(keyID)
            val downloadPath = prefs.getString(key, null)
            if (!downloadPath.isNullOrEmpty()) return
            val spEditor = prefs.edit()
            spEditor.putString(key, getVoiVistaChildFolderPathForDir(getDir(defaultDirectoryName)))
            spEditor.apply()
        }
    }
    ...
    private fun getVoiVistaChildFolderPathForDir(dir: File): String {
        return File(dir, "VoiVista").toURI().toString()
    }
    ...
    fun useStorageAccessFramework(context: Context): Boolean {
        ...
        // remote (see #6455).
        // TODO: if true, always get no app on device can open this
    ...
    private fun showSearchSuggestions(context: Context, sharedPreferences: SharedPreferences, @StringRes key: Int): Boolean {
        val enabledSearchSuggestions = sharedPreferences.getStringSet(context.getString(R.string.show_search_suggestions_key), null)
        return enabledSearchSuggestions?.contains(context.getString(key)) ?: true // defaults to true
    }

    @JvmStatic
    fun showLocalSearchSuggestions(context: Context, sharedPreferences: SharedPreferences): Boolean {
        return showSearchSuggestions(context, sharedPreferences, R.string.show_local_search_suggestions_key)
    }

    @JvmStatic
    fun showRemoteSearchSuggestions(context: Context, sharedPreferences: SharedPreferences): Boolean {
        return showSearchSuggestions(context, sharedPreferences, R.string.show_remote_search_suggestions_key)
    }

    private fun disableMediaTunnelingIfNecessary(context: Context, isFirstRun: Boolean) {
        val prefs = PreferenceManager.getDefaultSharedPreferences(context)
        val disabledTunnelingKey = context.getString(R.string.disable_media_tunneling_key)
        val disabledTunnelingAutomaticallyKey = context.getString(R.string.disabled_media_tunneling_automatically_key)
        val blacklistVersionKey = context.getString(R.string.media_tunneling_device_blacklist_version)

        val lastMediaTunnelingUpdate = prefs.getInt(blacklistVersionKey, 0)
        val wasDeviceBlacklistUpdated = DeviceUtils.MEDIA_TUNNELING_DEVICE_BLACKLIST_VERSION != lastMediaTunnelingUpdate
        val wasMediaTunnelingEnabledByUser = (prefs.getInt(disabledTunnelingAutomaticallyKey, -1) == 0 && !prefs.getBoolean(disabledTunnelingKey, false))

        if (java.lang.Boolean.TRUE == isFirstRun || (wasDeviceBlacklistUpdated && !wasMediaTunnelingEnabledByUser)) setMediaTunneling(context)
    }
    ...
    fun setMediaTunneling(context: Context) {
        val prefs = PreferenceManager.getDefaultSharedPreferences(context)
        if (!shouldSupportMediaTunneling()) prefs.edit()
            ...
            .putInt(context.getString(R.string.media_tunneling_device_blacklist_version), DeviceUtils.MEDIA_TUNNELING_DEVICE_BLACKLIST_VERSION)
            .apply()
        else prefs.edit().putInt(context.getString(R.string.media_tunneling_device_blacklist_version), DeviceUtils.MEDIA_TUNNELING_DEVICE_BLACKLIST_VERSION).apply()
    }
}
