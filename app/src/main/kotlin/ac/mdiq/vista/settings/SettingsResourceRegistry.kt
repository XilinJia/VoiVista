...
import ac.mdiq.vista.settings.fragment.DownloadSettingsFragment
import ac.mdiq.vista.settings.fragment.MainSettingsFragment
import ac.mdiq.vista.settings.fragment.*
import java.util.*
...
class SettingsResourceRegistry private constructor() {
    ...
    private fun add(fragmentClass: Class<out Fragment?>, @XmlRes preferencesResId: Int): SettingRegistryEntry {
        val entry = SettingRegistryEntry(fragmentClass, preferencesResId)
        registeredEntries.add(entry)
        return entry
    }

    fun getEntryByFragmentClass(fragmentClass: Class<out Fragment?>): SettingRegistryEntry? {
        Objects.requireNonNull(fragmentClass)
        return registeredEntries.stream().filter { e: SettingRegistryEntry? -> e!!.fragmentClass == fragmentClass }.findFirst().orElse(null)
    }

    fun getEntryByPreferencesResId(@XmlRes preferencesResId: Int): SettingRegistryEntry? {
        return registeredEntries.stream().filter { e: SettingRegistryEntry? -> e!!.preferencesResId == preferencesResId }.findFirst().orElse(null)
    }
    ...
    class SettingRegistryEntry(fragmentClass: Class<out Fragment?>, @field:XmlRes @param:XmlRes val preferencesResId: Int) {
