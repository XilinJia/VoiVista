...
import ac.mdiq.vista.util.ListHelper.getSortedResolutionList
import ac.mdiq.vista.util.ListHelper.isHighResolutionSelected
import ac.mdiq.vista.util.PermissionHelper.checkSystemAlertWindowPermission
import java.util.*
...
class VideoAudioSettingsFragment : BasePreferenceFragment() {
    ...
    override fun onCreatePreferences(savedInstanceState: Bundle?, rootKey: String?) {
        ...
        listener = OnSharedPreferenceChangeListener { sharedPreferences: SharedPreferences, key: String? ->
            ...
            when {
                Build.VERSION.SDK_INT >= Build.VERSION_CODES.M && getString(R.string.minimize_on_exit_key) == key -> {
                    val newSetting = sharedPreferences.getString(key, null)
                    if (newSetting != null && newSetting == getString(R.string.minimize_on_exit_popup_key) && !Settings.canDrawOverlays(context))
                        Snackbar.make(listView, R.string.permission_display_over_apps, Snackbar.LENGTH_INDEFINITE)
                            .setAction(R.string.settings) { view: View? -> checkSystemAlertWindowPermission(requireContext()) }.show()
                }
                getString(R.string.use_inexact_seek_key) == key -> updateSeekOptions()
                getString(R.string.show_higher_resolutions_key) == key -> updateResolutionOptions()
            }
        }
    }
    ...
    private fun updateResolutionOptions() {
        ...
        val resolutionListDescriptions = getSortedResolutionList(resources, R.array.resolution_list_description, R.array.high_resolution_list_descriptions, showHigherResolutions)
        val resolutionListValues = getSortedResolutionList(resources, R.array.resolution_list_values, R.array.high_resolution_list_values, showHigherResolutions)
        val limitDataUsageResolutionValues = getSortedResolutionList(resources, R.array.limit_data_usage_values_list, R.array.high_resolution_limit_data_usage_values_list, showHigherResolutions)
        val limitDataUsageResolutionDescriptions = getSortedResolutionList(resources, R.array.limit_data_usage_description_list, R.array.high_resolution_list_descriptions, showHigherResolutions)

        // get resolution preferences
        val defaultResolution = findPreference<ListPreference>(getString(R.string.default_resolution_key))
        val defaultPopupResolution = findPreference<ListPreference>(getString(R.string.default_popup_resolution_key))
        val mobileDataResolution = findPreference<ListPreference>(getString(R.string.limit_mobile_data_usage_key))
        ...
        if (!showHigherResolutions) {
            if (isHighResolutionSelected(defaultResolution.value, R.array.high_resolution_list_values, resources)) defaultResolution.setValueIndex(0)
            if (isHighResolutionSelected(defaultPopupResolution.value, R.array.high_resolution_list_values, resources)) defaultPopupResolution.setValueIndex(0)
            if (isHighResolutionSelected(mobileDataResolution.value, R.array.high_resolution_limit_data_usage_values_list, resources)) mobileDataResolution.setValueIndex(0)
        }
    }
    ...
    private fun updateSeekOptions() {
        ...
        for (durationsValue in durationsValues) {
            ...
            if (inexactSeek && currentDurationValue % 10 == 5) continue

            displayedDurationValues.add(durationsValue)
            try {
                displayedDescriptionValues.add(String.format(res.getQuantityString(R.plurals.seconds, currentDurationValue), currentDurationValue))
        ...
        val durations = findPreference<ListPreference>(getString(R.string.seek_duration_key))
        ...
        if (inexactSeek && selectedDuration / DateUtils.SECOND_IN_MILLIS.toInt() % 10 == 5) {
            ...
            val toast = Toast.makeText(context, getString(R.string.new_seek_duration_toast, newDuration), Toast.LENGTH_LONG)
            toast.show()
        }
    }
