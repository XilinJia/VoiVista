...
import ac.mdiq.vista.databinding.FragmentInstanceListBinding
import ac.mdiq.vista.databinding.ItemInstanceBinding
import ac.mdiq.vista.extractor.services.peertube.PeertubeInstance
import ac.mdiq.vista.util.KEY_MAIN_PAGE_CHANGE
...
class PeertubeInstanceListFragment : Fragment() {
    ...
    override fun onResume() {
        super.onResume()
        setTitleToAppCompatActivity(activity, getString(R.string.peertube_instance_url_title))
    }
    ...
    private fun addInstance(url: String) {
        ...
        val disposable = Single.fromCallable {
            ...
            .subscribe({ instance: PeertubeInstance ->
                ...
                binding!!.loadingProgressBar.visibility = View.GONE
                Toast.makeText(activity, R.string.peertube_instance_add_fail, Toast.LENGTH_SHORT).show()
            })
    ...
    private fun cleanUrl(url: String): String? {
        ...
        if (!cleanUrl.startsWith("https://")) {
            Toast.makeText(activity, R.string.peertube_instance_add_https_only, Toast.LENGTH_SHORT).show()
            return null
        }
        // only allow if not already exists
        for (instance in instanceListAdapter!!.currentList) {
            if (instance!!.url == cleanUrl) {
                Toast.makeText(activity, R.string.peertube_instance_add_exists, Toast.LENGTH_SHORT).show()
                return null
            }
        }
        return cleanUrl
    ...
    private val itemTouchCallback: ItemTouchHelper.SimpleCallback
        get() = object : ItemTouchHelper.SimpleCallback(ItemTouchHelper.UP or ItemTouchHelper.DOWN,
            ...
            override fun onMove(recyclerView: RecyclerView, source: RecyclerView.ViewHolder, target: RecyclerView.ViewHolder): Boolean {
                if (source.itemViewType != target.itemViewType || instanceListAdapter == null) return false
            ...
            override fun onSwiped(viewHolder: RecyclerView.ViewHolder, swipeDir: Int) {
                ...
                if (instanceListAdapter!!.currentList[position]!!.url == selectedInstance!!.url) {
                ...
                if (list.isEmpty()) list.add(selectedInstance)
    ...
    private inner class InstanceListAdapter(context: Context?, private val itemTouchHelper: ItemTouchHelper?)
        ...
        override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): TabViewHolder {
            return TabViewHolder(ItemInstanceBinding.inflate(inflater, parent, false))
        }
        override fun onBindViewHolder(holder: TabViewHolder, position: Int) {
            holder.bind(position)
        }

        inner class TabViewHolder(private val itemBinding: ItemInstanceBinding) : RecyclerView.ViewHolder(itemBinding.root) {
            ...
            fun bind(position: Int) {
                ...
                if (selectedInstance!!.url == instance.url) {
                    if (lastChecked != null && lastChecked !== itemBinding.selectInstanceRB) lastChecked!!.isChecked = false
    ...
    private class PeertubeInstanceCallback : DiffUtil.ItemCallback<PeertubeInstance>() {
        override fun areItemsTheSame(oldItem: PeertubeInstance, newItem: PeertubeInstance): Boolean {
            return oldItem.url == newItem.url
        }
        override fun areContentsTheSame(oldItem: PeertubeInstance, newItem: PeertubeInstance): Boolean {
