...
class LocalPlaylistManager(private val database: AppDatabase) {
    ...
    fun appendToPlaylist(playlistId: Long, streams: List<StreamEntity>): Maybe<List<Long>> {
        return playlistStreamTable.getMaximumIndexOf(playlistId)
            .firstElement()
            .map { maxJoinIndex: Int -> database.runInTransaction<List<Long>> {
                val streamIds = streamTable.upsertAll(streams)
    ...
    fun getPlaylistDuplicates(streamUrl: String?): Flowable<List<PlaylistDuplicatesEntry>> {
        return playlistStreamTable.getPlaylistDuplicatesMetadata(streamUrl).subscribeOn(Schedulers.io())
    }
    ...
    fun changePlaylistThumbnail(playlistId: Long, thumbnailStreamId: Long, isPermanent: Boolean): Maybe<Int> {
    ...
    fun getAutomaticPlaylistThumbnailStreamId(playlistId: Long): Long {
        ...
        if (streamId < 0) return PlaylistEntity.DEFAULT_THUMBNAIL_ID
        return streamId
    }

    private fun modifyPlaylist(playlistId: Long, name: String?, thumbnailStreamId: Long, isPermanent: Boolean): Maybe<Int> {
        return playlistTable.getPlaylist(playlistId)
            ?.firstElement()
            ?.filter { playlistEntities: List<PlaylistEntity?> -> playlistEntities.isNotEmpty() }
