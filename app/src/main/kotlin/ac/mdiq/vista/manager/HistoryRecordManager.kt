...
class HistoryRecordManager(context: Context) {
    ...
    fun markAsWatched(info: StreamInfoItem): Maybe<Long> {
        ...
        return Maybe.fromCallable {
            database.runInTransaction<Long> {
                ...
                if (info.duration < 0) {
                    val completeInfo = getStreamInfo(info.serviceId, info.url, false).subscribeOn(Schedulers.io()).blockingGet()
                ...
                val entity = StreamStateEntity(streamId, duration * 1000)
                ...
                if (latestEntry == null) return@runInTransaction streamHistoryTable.insert(StreamHistoryEntity(streamId, currentTime, 0))
                else return@runInTransaction 0L
    ...
    fun deleteCompleteStreamStateHistory(): Single<Int> {
        return Single.fromCallable { streamStateTable.deleteAll() }.subscribeOn(Schedulers.io())
    }
    ...
    fun onSearched(serviceId: Int, search: String?): Maybe<Long> {
        ...
        return Maybe.fromCallable {
            database.runInTransaction<Long> {
                ...
                if (latestEntry?.hasEqualValues(newEntry) == true) {
                    latestEntry.creationDate = currentTime
                    return@runInTransaction searchHistoryTable.update(latestEntry).toLong()
                } else return@runInTransaction searchHistoryTable.insert(newEntry)
            }
        }.subscribeOn(Schedulers.io())
    }
    ...
    fun getRelatedSearches(query: String, similarQueryLimit: Int, uniqueQueryLimit: Int): Flowable<List<String>> {
        val ret = if (query.isNotEmpty()) searchHistoryTable.getSimilarEntries(query, similarQueryLimit)
    ...
    fun loadStreamState(queueItem: PlayQueueItem): Maybe<StreamStateEntity> {
        return queueItem.stream
            .map { info: StreamInfo? -> streamTable.upsert(StreamEntity(info!!)) }
            .flatMapPublisher { streamId: Long? -> streamStateTable.getState(streamId!!) }
            .firstElement()
            .flatMap { list: List<StreamStateEntity> -> if (list.isEmpty()) Maybe.empty() else Maybe.just(list[0]) }
    ...
    fun saveStreamState(info: StreamInfo, progressMillis: Long): Completable {
        return Completable.fromAction {
            database.runInTransaction {
                ...
                if (state.isValid(info.duration)) streamStateTable.upsert(state)
            }
        }.subscribeOn(Schedulers.io())
    }
    ...
    fun loadStreamState(info: InfoItem): Single<Array<StreamStateEntity>> {
        return Single.fromCallable<Array<StreamStateEntity>> {
            val entities = streamTable.getStream(info.serviceId.toLong(), info.url).blockingFirst()
            if (entities.isEmpty()) return@fromCallable arrayOf<StreamStateEntity>()
            val states = streamStateTable.getState(entities[0].uid).blockingFirst()
            if (states.isEmpty()) return@fromCallable arrayOf<StreamStateEntity>()
            arrayOf(states[0])
        }.subscribeOn(Schedulers.io())
    }
...
//                    }
                ...
                val states = streamStateTable.getState(streamId).blockingFirst()
                if (states.isEmpty()) result.add(null)
                else result.add(states[0])
            }
            result
        }.subscribeOn(Schedulers.io())
    }
