...
object Utility {
    ...
    fun formatSpeed(speed: Double): String {
        val locale = Locale.getDefault()
        return when {
            speed < 1024 -> String.format(locale, "%.2f B/s", speed)
    ...
    fun getFileExt(url: String): String? {
        var url = url
        var index: Int
        if ((url.indexOf("?").also { index = it }) > -1) url = url.substring(0, index)
    ...
    fun getFileType(kind: Char, file: String): FileType {
        ...
        return when {
            file.endsWith(".srt") || file.endsWith(".vtt") || file.endsWith(".ssa") -> FileType.SUBTITLE
            file.endsWith(".mp3") || file.endsWith(".wav") || file.endsWith(".flac") || file.endsWith(".m4a")
                    || file.endsWith(".opus") -> FileType.MUSIC
            file.endsWith(".mp4") || file.endsWith(".mpeg") || file.endsWith(".rm") || file.endsWith(".rmvb")
                    || file.endsWith(".flv") || file.endsWith(".webp") || file.endsWith(".webm") -> FileType.VIDEO
    ...
    fun checksum(source: StoredFileHelper, algorithmId: Int): String {
        ...
        when (algorithmId) {
            R.id.md5 -> byteString = byteString.md5()
            R.id.sha1 -> byteString = byteString.sha1()
        }
    ...
    fun getTotalContentLength(connection: HttpURLConnection): Long {
        try {
            ...
            } else return getContentLength(connection)
        } catch (err: Exception) {
            // nothing to do
        }
        return -1
