...
import ac.mdiq.vista.util.FilePickerActivityHelper
...
class StoredDirectoryHelper(private val context: Context, path: Uri, @JvmField val tag: String) {
    private var ioTree: Path? = null
    private val docTree: DocumentFile? = DocumentFile.fromTreeUri(context, path)

    init {

        if (ContentResolver.SCHEME_FILE.equals(path.scheme, ignoreCase = true)) ioTree = Paths.get(URI.create(path.toString()))
        else {
            try {
            ...
            if (this.docTree == null) throw IOException("Failed to create the tree from Uri")
        }
    }
    ...
    fun createUniqueFile(name: String, mime: String): StoredFileHelper? {
        ...
        if (docTree == null) {
            try {
                Files.list(ioTree).use { stream ->
                    matches.addAll(stream.map { path: Path -> path.fileName.toString().lowercase(Locale.getDefault()) }
            ...
            val docTreeChildren = DocumentsContract.buildChildDocumentsUriUsingTree(docTree.uri, DocumentsContract.getDocumentId(docTree.uri))
        ...
        if (matches.isEmpty()) return createFile(name, mime, true)
    ...
    private fun createFile(filename: String, mime: String, safe: Boolean): StoredFileHelper? {
        ...
        try {
            storage = if (docTree == null && ioTree != null) StoredFileHelper(ioTree!!, filename, mime)
            else StoredFileHelper(context, docTree, filename, mime, safe)
        } catch (e: IOException) {
            return null
        }
    ...
    val isInvalidSafStorage: Boolean
         ...
         * Apps & notifications -> Vista -> Storage & cache -> Clear access`);
         */
    ...
    companion object {
        ...
        const val PERMISSION_FLAGS: Int = (Intent.FLAG_GRANT_READ_URI_PERMISSION or Intent.FLAG_GRANT_WRITE_URI_PERMISSION)
        ...
        private fun addIfStartWith(list: MutableList<String>, base: String, str: String) {
            ...
            if (lowerStr.startsWith(base)) list.add(lowerStr)
        }
        ...
        private fun splitFilename(filename: String): Array<String> {
            ...
            if (dotIndex < 0 || (dotIndex == filename.length - 1)) return arrayOf(filename, "")
        ...
        fun findFileSAFHelper(context: Context?, tree: DocumentFile, filename: String): DocumentFile? {
            ...
            val childrenUri = DocumentsContract.buildChildDocumentsUriUsingTree(tree.uri, DocumentsContract.getDocumentId(tree.uri))
            val projection = arrayOf(DocumentsContract.Document.COLUMN_DISPLAY_NAME, DocumentsContract.Root.COLUMN_DOCUMENT_ID)
            val contentResolver = context.contentResolver

            val lowerFilename = filename.lowercase(Locale.getDefault())
            contentResolver.query(childrenUri, projection, selection,
                arrayOf(lowerFilename), null).use { cursor ->
                if (cursor == null) return null

                while (cursor.moveToNext()) {
                    if (cursor.isNull(name) || !cursor.getString(name).lowercase(Locale.getDefault()).startsWith(lowerFilename)) continue
                    return DocumentFile.fromSingleUri(context, DocumentsContract.buildDocumentUriUsingTree(tree.uri, cursor.getString(documentId)))
                }
            }
            return null
        }
        ...
        fun getPicker(ctx: Context): Intent {
            return if (VistaSettings.useStorageAccessFramework(ctx)) Intent(Intent.ACTION_OPEN_DOCUMENT_TREE)
                .putExtra("android.content.extra.SHOW_ADVANCED", true)
                .addFlags(Intent.FLAG_GRANT_PERSISTABLE_URI_PERMISSION or PERMISSION_FLAGS)
            else Intent(ctx, FilePickerActivityHelper::class.java)
