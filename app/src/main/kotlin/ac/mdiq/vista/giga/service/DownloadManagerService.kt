package ac.mdiq.vista.giga.service
...
import ac.mdiq.vista.ui.activity.DownloadActivity
import ac.mdiq.vista.giga.io.StoredDirectoryHelper
import ac.mdiq.vista.giga.io.StoredFileHelper
import ac.mdiq.vista.util.Localization.downloadCount
...
import ac.mdiq.vista.giga.Postprocessing.Companion.getAlgorithm
import ac.mdiq.vista.giga.service.DownloadManager.Companion.pickAvailableTemporalDir
import java.io.File
...
class DownloadManagerService : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        Logd(TAG, "onStartCommand " + if (intent == null) "Restarting" else "Starting")
        if (intent == null) return START_NOT_STICKY

        Log.i(TAG, "onStartCommand Got intent: $intent")
        val action = intent.action
        if (action != null) {
            if (action == Intent.ACTION_RUN) mHandler!!.post { startMission(intent) }
                ...
                if (action == ACTION_OPEN_DOWNLOADS_FINISHED)
                    startActivity(Intent(this, DownloadActivity::class.java).setAction(Intent.ACTION_MAIN).addFlags(Intent.FLAG_ACTIVITY_NEW_TASK))
    ...
    override fun onDestroy() {
        ...
        if (mNotificationManager != null && downloadDoneNotification != null) {
            downloadDoneNotification!!.setDeleteIntent(null) // prevent Vista running when is killed, cleared from recent, etc
    ...
    private fun handlePreferenceChange(prefs: SharedPreferences?, key: String) {
        when (key) {
            ...
            getString(R.string.downloads_cross_network) -> mManager.mPrefMeteredDownloads = prefs?.getBoolean(key, false) ?: false
            getString(R.string.downloads_queue_limit) -> mManager.mPrefQueueLimit = prefs?.getBoolean(key, true) ?: true
            getString(R.string.download_path_video_key) -> mManager.mMainStorageVideo = loadMainVideoStorage()
            getString(R.string.download_path_audio_key) -> mManager.mMainStorageAudio = loadMainAudioStorage()
        }
    }

    private fun updateForegroundState(state: Boolean) {
...
//            startForeground(FOREGROUND_NOTIFICATION_ID, mNotification)
            ...
            startService(intent)
        } else ServiceCompat.stopForeground(this, ServiceCompat.STOP_FOREGROUND_REMOVE)
    ...
    private fun notifyFinishedDownload(name: String?) {
    ...
    private fun notifyFailedDownload(mission: DownloadMission) {
    ...
    inner class DownloadManagerBinder : Binder() {
        ...
        fun askForSavePath(): Boolean {
            return mPrefs.getBoolean(this@DownloadManagerService.getString(R.string.downloads_storage_ask), false)
        }
    ...
    companion object {
         ...
         */
        @JvmStatic
        fun startMission(context: Context, urls: Array<String>, storage: StoredFileHelper, kind: Char, threads: Int, source: String?, psName: String?,
                         psArgs: Array<String?>?, nearLength: Long, recoveryInfo: ArrayList<MissionRecoveryInfo?>?) {
            Logd(TAG, "startMission: urls: ${urls.size}")
