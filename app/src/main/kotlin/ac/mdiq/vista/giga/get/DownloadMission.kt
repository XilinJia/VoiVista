package ac.mdiq.vista.giga.get
...
class DownloadMission(
        ...
        kind: Char,
        psInstance: Postprocessing?) : Mission() {
    /**
    ...
    fun writeThisToFile() {
        synchronized(LOCK) {
            if (metadata == null) return
//            Logd(TAG, "writeThisToFile: $metadata")
            ...
            while (true) {
                try {
                    ...
                    if (!mMission.unknownLength && mMission.recoveryInfo != null) {
                        ...
                        when {
                            !entityTag.isNullOrEmpty() -> recovery.validateCondition = entityTag
                            !lastModified.isNullOrEmpty() -> recovery.validateCondition = lastModified // Note: this is less precise
    ...
    class DownloadRunnable internal constructor(mission: DownloadMission, private val mId: Int) : Thread() {
        ...
        override fun run() {
            ...
            while (mMission.running && mMission.errCode == ERROR_NOTHING) {
                ...
                if (block == null) {
                    Logd(TAG, "run() $mId:no more blocks left, exiting")
                    break
                }

                if (retry) Logd(TAG, "run() $mId:retry block at position=${block.position} from the start")
                else Logd(TAG, "run() $mId:acquired block at position=${block.position} done=${block.done}")
...
//                            Logd(TAG, "start: $start len: $len")
                    ...
                    if (mMission.running) Logd(TAG, "run() $mId:position ${block.position} stopped $start/$end")
                } catch (e: Exception) {
            ...
            f.close()
            Logd(TAG, "run() thread $mId exited from main download loop")
            if (mMission.errCode == ERROR_NOTHING && mMission.running) {
                Logd(TAG, "run() no error has happened, notifying")
                mMission.notifyFinished()
            }
            if (!mMission.running) Logd(TAG, "run() The mission has been paused. Passing.")
        }
    ...
    class DownloadMissionRecover internal constructor(private val mMission: DownloadMission, private val mErrCode: Int) : Thread() {
        ...
        private fun tryRecover() {
            if (mExtractor == null && mMission.source != null) {
                try {
                    val svr = Vista.getServiceByUrl(mMission.source!!)
            ...
            when (mRecovery!!.kind) {
                ...
                'v' -> {
                    ...
                    for (video in videoStreams) {
                        if (video.resolution == mRecovery!!.desired && video.format == mRecovery!!.format && video.deliveryMethod == DeliveryMethod.PROGRESSIVE_HTTP) {
                            url = video.content
                            break
                        }
                    }
                ...
                's' -> {
                    val format = mRecovery!!.format ?: throw RuntimeException("Unknown stream type")
                    for (subtitles in mExtractor!!.getSubtitles(format)) {
