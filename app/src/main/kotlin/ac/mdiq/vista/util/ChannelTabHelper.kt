...
object ChannelTabHelper {
    ...
    fun isStreamsTab(tab: ListLinkHandler): Boolean {
        ...
        return if (contentFilters.isEmpty()) false else { isStreamsTab(contentFilters[0]) }
    }
    ...
    fun showChannelTab(context: Context, sharedPreferences: SharedPreferences, @StringRes key: Int): Boolean {
        val enabledTabs = sharedPreferences.getStringSet(context.getString(R.string.show_channel_tabs_key), null)
    ...
    fun fetchFeedChannelTab(context: Context, sharedPreferences: SharedPreferences, tab: ListLinkHandler): Boolean {
        val contentFilters = tab.contentFilters
        // this should never happen, but check just to be sure
        if (contentFilters.isEmpty()) return false

        val key = getFetchFeedTabKey(contentFilters[0])
        if (key == -1) return false

        val enabledTabs = sharedPreferences.getStringSet(context.getString(R.string.feed_fetch_channel_tabs_key), null)
        return enabledTabs?.contains(context.getString(key)) ?: true // default to true
    }
}
