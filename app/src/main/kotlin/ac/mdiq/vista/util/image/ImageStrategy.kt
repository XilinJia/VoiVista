...
object ImageStrategy {
    ...
    fun estimatePixelCount(image: Image, widthOverHeight: Double): Double {
        return when {
            ...
            image.height == Image.HEIGHT_UNKNOWN -> if (image.width == Image.WIDTH_UNKNOWN) 0.0 else image.width * image.width / widthOverHeight
            image.width == Image.WIDTH_UNKNOWN -> image.height * image.height * widthOverHeight
            else -> (image.height * image.width).toDouble()
        }
    }
    ...
    fun choosePreferredImage(images: List<Image>, nonNoneQuality: PreferredImageQuality): String? {
        ...
        val widthOverHeight = images.stream()
            .filter { image: Image -> (image.height != Image.HEIGHT_UNKNOWN && image.width != Image.WIDTH_UNKNOWN) }
        ...
        val initialComparator = Comparator // the first step splits the images into groups of resolution levels
            .comparingInt { i: Image ->
                when (i.estimatedResolutionLevel) {
                    ResolutionLevel.UNKNOWN -> return@comparingInt 3 /* avoid unknowns as much as possible */
                    preferredLevel -> return@comparingInt 0 // prefer a matching resolution level
                    ResolutionLevel.MEDIUM -> return@comparingInt 1 // the preferredLevel is only 1 "step" away (either HIGH or LOW)
                    else -> return@comparingInt 2 // the preferredLevel is the furthest away possible (2 "steps")
                }
    ...
    fun choosePreferredImage(images: List<Image>): String? {
        // do not load images
        if (preferredImageQuality == PreferredImageQuality.NONE) return null
    ...
    fun imageListToDbUrl(images: List<Image>): String? {
        val quality = if (preferredImageQuality == PreferredImageQuality.NONE) PreferredImageQuality.MEDIUM else preferredImageQuality
    ...
    fun dbUrlToImageList(url: String?): List<Image> {
        return if (url == null) listOf() else  listOf(Image(url, -1, -1, ResolutionLevel.UNKNOWN))
    }
}
