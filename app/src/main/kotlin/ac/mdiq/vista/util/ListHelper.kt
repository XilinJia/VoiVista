...
object ListHelper {
    ...
    private val VIDEO_FORMAT_QUALITY_RANKING: List<MediaFormat?> = listOf(MediaFormat.v3GPP, MediaFormat.WEBM, MediaFormat.MPEG_4)

    // Audio format in order of quality. 0=lowest quality, n=highest quality
    private val AUDIO_FORMAT_QUALITY_RANKING: List<MediaFormat> = listOf(MediaFormat.MP3, MediaFormat.WEBMA, MediaFormat.M4A)

    // Audio format in order of efficiency. 0=least efficient, n=most efficient
    private val AUDIO_FORMAT_EFFICIENCY_RANKING: List<MediaFormat?> = listOf(MediaFormat.MP3, MediaFormat.M4A, MediaFormat.WEBMA)
    ...
    private val AUDIO_TRACK_TYPE_RANKING: List<AudioTrackType> = listOf(AudioTrackType.DESCRIPTIVE, AudioTrackType.DUBBED, AudioTrackType.ORIGINAL)

    // Audio track types in order of priority when descriptive audio is preferred.
    private val AUDIO_TRACK_TYPE_RANKING_DESCRIPTIVE: List<AudioTrackType?> = listOf(AudioTrackType.ORIGINAL, AudioTrackType.DUBBED, AudioTrackType.DESCRIPTIVE)

    /**
     * List of supported YouTube Itag ids.
     * The original order is kept.
     * @see {@link ac.mdiq.vista.extractor.services.youtube.ItagItem.ITAG_LIST}
     */
    ...
    fun getDefaultAudioFormat(context: Context, audioStreams: List<AudioStream?>?): Int {
        return getAudioIndexByHighestRank(audioStreams, getAudioTrackComparator(context).thenComparing(
    ...
    fun getDefaultAudioTrackGroup(context: Context, groupedAudioStreams: List<List<AudioStream?>?>?): Int {
        if (groupedAudioStreams.isNullOrEmpty()) return -1
    ...
    fun getAudioFormatIndex(context: Context, audioStreams: List<AudioStream?>, trackId: String?): Int {
        if (trackId != null) {
            for (i in audioStreams.indices) {
                ...
                if (s!!.audioTrackId != null && s.audioTrackId == trackId) return i
            }
        }
    ...
    fun <S : Stream?> getStreamsOfSpecifiedDelivery(streamList: List<S>?, deliveryMethod: DeliveryMethod): List<S> {
        return getFilteredStreamList(streamList) { stream: S -> stream!!.deliveryMethod == deliveryMethod }
    }
    ...
    fun <S : Stream?> getUrlAndNonTorrentStreams(streamList: List<S>?): List<S> {
        return getFilteredStreamList(streamList) { stream: S -> stream!!.isUrl && stream.deliveryMethod != DeliveryMethod.TORRENT }
    }
    ...
    fun <S : Stream?> getPlayableStreams(streamList: List<S>?, serviceId: Int): List<S> {
        ...
        return getFilteredStreamList(streamList) { stream: S ->
            (stream!!.deliveryMethod != DeliveryMethod.TORRENT && (stream.deliveryMethod != DeliveryMethod.HLS || stream.format != MediaFormat.OPUS)
                    && (serviceId != youtubeServiceId || stream.getItagItem() == null || SUPPORTED_ITAG_IDS.contains(stream.getItagItem()!!.id)))
        }
    }
     ...
     */
    fun getSortedStreamVideosList(context: Context, videoStreams: List<VideoStream>?, videoOnlyStreams: List<VideoStream>?,
                                  ascendingOrder: Boolean, preferVideoOnlyStreams: Boolean): List<VideoStream> {
        ...
        return getSortedStreamVideosList(defaultFormat, showHigherResolutions, videoStreams, videoOnlyStreams, ascendingOrder, preferVideoOnlyStreams)
    }
     ...
     */
    @JvmStatic
    fun getSortedResolutionList(resources: Resources, defaultResolutionKey: Int, additionalResolutionKey: Int, showHigherResolutions: Boolean): List<String> {
        val resolutions: MutableList<String> = mutableListOf(*resources.getStringArray(defaultResolutionKey))
        if (!showHigherResolutions) return resolutions
        val additionalResolutions = listOf(*resources.getStringArray(additionalResolutionKey))
    ...
    @JvmStatic
    fun isHighResolutionSelected(selectedResolution: String, additionalResolutionKey: Int, resources: Resources): Boolean {
        return listOf(*resources.getStringArray(additionalResolutionKey)).contains(selectedResolution)
    }
    ...
    fun getFilteredAudioStreams(context: Context, audioStreams: List<AudioStream>?): List<AudioStream?> {
        if (audioStreams == null) return emptyList<AudioStream>()
        ...
        for (stream in audioStreams) {
            ...
            if (presentStream == null || cmp.compare(stream, presentStream) > 0) collectedStreams[trackId] = stream
        }

        // Filter unknown audio tracks if there are multiple tracks
        if (collectedStreams.size > 1) collectedStreams.remove("")

        // Sort collected streams by name
        return collectedStreams.values.stream().sorted(getAudioTrackNameComparator(context)).collect(Collectors.toList())
    }
    ...
    fun getGroupedAudioStreams(context: Context, audioStreams: List<AudioStream>?): List<List<AudioStream>> {
        if (audioStreams == null) return emptyList()

        val collectedStreams = HashMap<String, MutableList<AudioStream>>()

        for (stream in audioStreams) {
            val trackId = Objects.toString(stream.audioTrackId, "")
            if (collectedStreams.containsKey(trackId)) collectedStreams[trackId]!!.add(stream)
            else {
    ...
    private fun <S : Stream?> getFilteredStreamList(streamList: List<S>?, streamListPredicate: Predicate<S>): List<S> {
        if (streamList == null) return emptyList()
    ...
    private fun computeDefaultResolution(context: Context, key: Int, value: Int): String? {
        ...
        if (maxResolution != null && (resolution == context.getString(R.string.best_resolution_key) || compareVideoStreamResolution(maxResolution, resolution!!) < 1))
     ...
     */
    fun getDefaultResolutionIndex(defaultResolution: String, bestResolutionKey: String, defaultFormat: MediaFormat?, videoStreams: List<VideoStream>?): Int {
     ...
     * @return the sorted list
     */
    fun getSortedStreamVideosList(defaultFormat: MediaFormat?, showHigherResolutions: Boolean, videoStreams: List<VideoStream>?,
                                  videoOnlyStreams: List<VideoStream>?, ascendingOrder: Boolean, preferVideoOnlyStreams: Boolean): List<VideoStream> {
        // Determine order of streams
        // The last added list is preferred
        val videoStreamsOrdered = if (preferVideoOnlyStreams) listOf(videoStreams, videoOnlyStreams)
        else listOf(videoOnlyStreams, videoStreams)
        ...
        val allInitialStreams = videoStreamsOrdered.stream() // Ignore lists that are null
            ...
            .filter { stream: VideoStream ->
                ...
                (showHigherResolutions || !HIGH_RESOLUTION_LIST.contains(stream.resolution.replace("p\\d+$".toRegex(), "p"))) }
            .collect(Collectors.toList())

        val hashMap = HashMap<String, VideoStream>()
        // Add all to the hashmap
        for (videoStream in allInitialStreams) hashMap[videoStream.resolution] = videoStream

        // Override the values when the key == resolution, with the defaultFormat
        for (videoStream in allInitialStreams) {
            if (videoStream.format == defaultFormat) hashMap[videoStream.resolution] = videoStream
        }
     ...
     */
    private fun sortStreamList(videoStreams: List<VideoStream>, ascendingOrder: Boolean): List<VideoStream> {
        // Compares the quality of two video streams.
        val comparator = Comparator.nullsLast(Comparator
            .comparing({ obj: VideoStream -> obj.resolution }, { obj: String?, r1: String? ->
    ...
    fun getAudioIndexByHighestRank(audioStreams: List<AudioStream?>?, comparator: Comparator<AudioStream?>?): Int {
        if (audioStreams.isNullOrEmpty()) return -1
        val highestRankedAudioStream = audioStreams.stream().max(comparator).orElse(null)
     ...
     */
    fun getVideoStreamIndex(targetResolution: String, targetFormat: MediaFormat?, videoStreams: List<VideoStream>): Int {
        ...
        for (idx in videoStreams.indices) {
            ...
            if (format == targetFormat && resolution == targetResolution) fullMatchIndex = idx
            if (format == targetFormat && resolutionNoRefresh == targetResolutionNoRefresh) fullMatchNoRefreshIndex = idx
            if (resMatchOnlyIndex == -1 && resolution == targetResolution) resMatchOnlyIndex = idx
            if (resMatchOnlyNoRefreshIndex == -1 && resolutionNoRefresh == targetResolutionNoRefresh) resMatchOnlyNoRefreshIndex = idx
    ...
    private fun getMediaFormatFromKey(context: Context, formatKey: String): MediaFormat? {
        ...
        when (formatKey) {
            context.getString(R.string.video_webm_key) -> format = MediaFormat.WEBM
            context.getString(R.string.video_mp4_key) -> format = MediaFormat.MPEG_4
            context.getString(R.string.video_3gp_key) -> format = MediaFormat.v3GPP
            context.getString(R.string.audio_webm_key) -> format = MediaFormat.WEBMA
            context.getString(R.string.audio_m4a_key) -> format = MediaFormat.M4A
        }
        return format
    }
    ...
    private fun compareVideoStreamResolution(r1: String, r2: String): Int {
        try {
            val res1 = r1.replace("0p\\d+$".toRegex(), "1").replace("[^\\d.]".toRegex(), "").toInt()
            val res2 = r2.replace("0p\\d+$".toRegex(), "1").replace("[^\\d.]".toRegex(), "").toInt()
            return res1 - res2
    ...
    fun isMeteredNetwork(context: Context): Boolean {
        ...
        if (manager == null || manager.activeNetworkInfo == null) return false
     ...
     * @return Comparator
     */
    private fun getAudioFormatComparator(context: Context): Comparator<AudioStream?> {
    ...
    fun getAudioFormatComparator(defaultFormat: MediaFormat?, limitDataUsage: Boolean): Comparator<AudioStream?> {
        ...
        if (limitDataUsage) bitrateComparator = bitrateComparator.reversed()

        return Comparator.comparing(
            { obj: AudioStream? -> obj!!.format }, { o1: MediaFormat?, o2: MediaFormat? ->
                if (defaultFormat != null) return@comparing java.lang.Boolean.compare(o1 == defaultFormat, o2 == defaultFormat)
                0
            }).thenComparing(bitrateComparator).thenComparingInt { stream: AudioStream? ->
            formatRanking.indexOf(stream!!.format)
        }
    }
     ...
     * @return Comparator
     */
    private fun getAudioTrackComparator(context: Context): Comparator<AudioStream?> {
        val preferences = PreferenceManager.getDefaultSharedPreferences(context)
        val preferredLanguage = Localization.getPreferredLocale(context)
        val preferOriginalAudio = preferences.getBoolean(context.getString(R.string.prefer_original_audio_key), false)
     ...
     */
    @JvmStatic
    fun getAudioTrackComparator(preferredLanguage: Locale, preferOriginalAudio: Boolean, preferDescriptiveAudio: Boolean): Comparator<AudioStream?> {
        ...
        return Comparator.comparing({ obj: AudioStream? -> obj!!.audioTrackType },
            { o1: AudioTrackType?, o2: AudioTrackType? ->
                if (preferOriginalAudio) return@comparing java.lang.Boolean.compare(o1 == AudioTrackType.ORIGINAL, o2 == AudioTrackType.ORIGINAL)
                0
     ...
     * @return Comparator
     */
    private fun getAudioTrackNameComparator(context: Context): Comparator<AudioStream?> {
        val appLoc = Localization.getAppLocale(context)
        return Comparator.comparing({ obj: AudioStream? -> obj?.audioLocale },
            Comparator.nullsLast(Comparator.comparing { locale: Locale? -> locale?.getDisplayName(appLoc)?:"" }))
            .thenComparing { obj: AudioStream? -> obj?.audioTrackType ?: AudioTrackType.ORIGINAL }
    }
}
