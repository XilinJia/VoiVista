 ...
 * StateSaver.kt is part of Vista
 *
 * License: GPL-3.0+
...
object StateSaver {
    ...
    fun tryToRestore(outState: Bundle?, writeRead: WriteRead?): SavedState? {
        ...
        val savedState = BundleCompat.getParcelable(outState, KEY_SAVED_STATE, SavedState::class.java) ?: return null
    ...
    private fun tryToSave(isChangingConfig: Boolean, prefixFileName: String, suffixFileName: String, writeRead: WriteRead): SavedState? {
        ...
        try {
            ...
            val file = File(cacheDir, prefixFileName + (suffixFileName.ifEmpty { ".cache" }))
            // If the file already exists, just return it
            if (file.exists() && file.length() > 0) return SavedState(prefixFileName, file.absolutePath)

            // Delete any file that contains the prefix
            val files = cacheDir.listFiles { _: File?, name: String -> name.contains(prefixFileName) }
