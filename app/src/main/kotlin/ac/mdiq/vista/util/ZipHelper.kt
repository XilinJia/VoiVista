...
object ZipHelper {
     ...
     */
    @Throws(Exception::class)
    fun extractFileFromZip(zipFile: StoredFileHelper, file: String, name: String): Boolean {
        ZipInputStream(BufferedInputStream(
            ...
            while ((inZip.nextEntry.also { ze = it }) != null) {
                if (ze.name == name) {
                    ...
                    if (oldFile.exists()) {
                        if (!oldFile.delete()) throw Exception("Could not delete $file")
                    }
    ...
    fun isValidZipFile(file: StoredFileHelper): Boolean {
        try {
            ZipInputStream(BufferedInputStream(
                SharpInputStream(file.stream))).use {
                return true
            }
