...
object ServiceHelper {
    ...
    fun getSelectedService(context: Context): StreamingService? {
        val serviceName = PreferenceManager.getDefaultSharedPreferences(context).getString(context.getString(R.string.current_service_key), context.getString(R.string.default_service_value))
        return try { Vista.getService(serviceName?:"") } catch (e: ExtractionException) { null }
    }
...
//            .findFirst()
//            .map { obj: StreamingService -> obj.serviceInfo }
//            .map { it.name }
//            .orElse("<unknown>")
        return ServiceList.all().firstOrNull { it.serviceId == serviceId }?.serviceInfo?.name ?: "<unknown>"
    }
...
//            .findFirst()
//            .orElseThrow()
        return ServiceList.all().firstOrNull { it.serviceId == serviceId } ?: throw RuntimeException("Service not found")
    }

    @JvmStatic
    fun setSelectedServiceId(context: Context, serviceId: Int) {
        val serviceName = try { Vista.getService(serviceId).serviceInfo.name }
        catch (e: ExtractionException) { DEFAULT_FALLBACK_SERVICE.serviceInfo.name }
    ...
    fun initService(context: Context, serviceId: Int) {
        if (serviceId == ServiceList.PeerTube.serviceId) {
            ...
            try { jsonObject = JsonParser.`object`().from(json) } catch (e: JsonParserException) { return }
