...
object PicassoHelper {
    ...
    fun init(context: Context) {
        ...
        picassoDownloaderClient = OkHttpClient.Builder()
            ...
            .cache(Cache(File(context.externalCacheDir, "picasso"), 50L * 1024L * 1024L))
    ...
    fun loadScaledDownThumbnail(context: Context, images: List<Image>): RequestCreator {
        ...
        return loadThumbnail(images)
            .transform(object : Transformation {
                override fun transform(source: Bitmap): Bitmap {
                    ...
                    val notificationThumbnailWidth = min(context.resources.getDimension(R.dimen.player_notification_thumbnail_width).toDouble(), source.width.toDouble()).toFloat()

                    val result = BitmapCompat.createScaledBitmap(source, notificationThumbnailWidth.toInt(),
                        (source.height / (source.width / notificationThumbnailWidth)).toInt(), null, true)

                    if (result == source || !result.isMutable) {
                        // create a new mutable bitmap to prevent strange crashes on some
                        // devices (see #4638)
                        val copied = BitmapCompat.createScaledBitmap(source, notificationThumbnailWidth.toInt() - 1,
                            (source.height / (source.width / (notificationThumbnailWidth - 1))).toInt(), null, true)
                        source.recycle()
                        return copied
                    } else {
                        source.recycle()
    ...
    private fun loadImageDefault(images: List<Image>, @DrawableRes placeholderResId: Int): RequestCreator {
        return loadImageDefault(choosePreferredImage(images), placeholderResId)
    }

    private fun loadImageDefault(url: String?, @DrawableRes placeholderResId: Int, showPlaceholderWhileLoading: Boolean = true): RequestCreator {
        ...
        if (url.isNullOrEmpty() || !shouldLoadImages()) {
            return picassoInstance!!
                .load(null as String?)
                .placeholder(placeholderResId) // show placeholder when no image should load
                .error(placeholderResId)
        } else {
            val requestCreator = picassoInstance!!.load(url).error(placeholderResId)
            if (showPlaceholderWhileLoading) requestCreator.placeholder(placeholderResId)
