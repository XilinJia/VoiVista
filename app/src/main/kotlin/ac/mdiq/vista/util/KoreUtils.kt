...
object KoreUtils {
    fun isServiceSupportedByKore(serviceId: Int): Boolean {
        return (serviceId == ServiceList.YouTube.serviceId || serviceId == ServiceList.SoundCloud.serviceId)
    }

    @JvmStatic
    fun shouldShowPlayWithKodi(context: Context, serviceId: Int): Boolean {
        return (isServiceSupportedByKore(serviceId) && PreferenceManager.getDefaultSharedPreferences(context)
    ...
    fun playWithKore(context: Context, streamUrl: Uri?) {
        ...
        if (!ShareUtils.tryOpenIntentInApp(context, intent)) {
            AlertDialog.Builder(context)
                ...
                .setPositiveButton(R.string.install) { _: DialogInterface?, _: Int -> installKore(context) }
