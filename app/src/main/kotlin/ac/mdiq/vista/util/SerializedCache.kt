...
class SerializedCache private constructor() {
    fun <T> take(key: String, type: Class<T>): T? {
        ...
        synchronized(LRU_CACHE) {
            return if (LRU_CACHE[key] != null) getItem(LRU_CACHE.remove(key)!!, type) else null
        }
    }
    ...
    private fun <T : Serializable> clone(item: T, type: Class<T>): T {
        ...
        val clone = ObjectInputStream(ByteArrayInputStream(bytesOutput.toByteArray())).readObject()
//        TODO: not quite proper
