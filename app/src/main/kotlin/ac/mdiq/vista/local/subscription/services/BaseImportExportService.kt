 ...
 * BaseImportExportService.kt is part of Vista
 *
 * License: GPL-3.0+
...
//        }
    ...
    protected fun updateNotification(text: String) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            if (text.isNotEmpty()) notificationBuilder!!.setContentText("$text  ($progressText)")
        } else {
    ...
    protected fun getErrorMessage(error: Throwable): String? {
        ...
        when {
            error is InvalidSourceException -> message = getString(R.string.invalid_source)
            error is FileNotFoundException -> message = getString(R.string.invalid_file)
            error.isNetworkRelated -> message = getString(R.string.network_error)
        }
        return message
    }
