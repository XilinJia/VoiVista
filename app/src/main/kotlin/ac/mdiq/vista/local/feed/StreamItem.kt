package ac.mdiq.vista.local.feed
...
import ac.mdiq.vista.databinding.ListStreamItemBinding
...
data class StreamItem(val streamWithState: StreamWithState, var itemVersion: ItemVersion = ItemVersion.NORMAL) : BindableItem<ListStreamItemBinding>() {
    ...
    override fun bind(viewBinding: ListStreamItemBinding, position: Int, payloads: MutableList<Any>) {
        if (payloads.contains(UPDATE_RELATIVE_TIME)) {
            if (itemVersion != ItemVersion.MINI) viewBinding.itemAdditionalDetails.text = getStreamInfoDetailLine(viewBinding.itemAdditionalDetails.context)
            return
        }
    ...
    override fun bind(viewBinding: ListStreamItemBinding, position: Int) {
        ...
        if (stream.duration > 0) {
            ...
            viewBinding.itemDurationView.setBackgroundColor(ContextCompat.getColor(viewBinding.itemDurationView.context, R.color.duration_background_color))
            ...
            } else viewBinding.itemProgressView.visibility = View.GONE
        } else if (StreamTypeUtil.isLiveStream(stream.streamType)) {
            viewBinding.itemDurationView.setText(R.string.duration_live)
            viewBinding.itemDurationView.setBackgroundColor(ContextCompat.getColor(viewBinding.itemDurationView.context, R.color.live_duration_background_color))
            viewBinding.itemDurationView.visibility = View.VISIBLE
