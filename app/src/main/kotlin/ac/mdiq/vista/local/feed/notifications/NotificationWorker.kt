...
class NotificationWorker(appContext: Context, workerParams: WorkerParameters) : RxWorker(appContext, workerParams) {
    ...
    private fun showLoadingFeedForegroundNotification() {
        val notification = NotificationCompat.Builder(applicationContext, applicationContext.getString(R.string.notification_channel_id))
            .setOngoing(true)
    ...
    companion object {
        ...
        fun initialize(context: Context) {
            if (areNotificationsEnabled(context)) schedule(context)
            else cancel(context)
        }
