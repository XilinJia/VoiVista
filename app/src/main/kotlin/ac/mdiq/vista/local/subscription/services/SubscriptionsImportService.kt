 ...
 * SubscriptionsImportService.kt is part of Vista
 *
 * License: GPL-3.0+
 ...
 */
package ac.mdiq.vista.local.subscription.services
...
import ac.mdiq.vista.giga.io.SharpInputStream
import ac.mdiq.vista.giga.io.StoredFileHelper
import ac.mdiq.vista.local.subscription.services.ImportExportJsonHelper.readFrom
...
class SubscriptionsImportService : BaseImportExportService() {
        ...
        get() = object : Subscriber<List<SubscriptionEntity>> {
            ...
            override fun onComplete() {
                LocalBroadcastManager.getInstance(this@SubscriptionsImportService)
                    .sendBroadcast(Intent(IMPORT_COMPLETE_ACTION).setPackage(packageName))
        ...
        get() = Consumer { notification: Notification<Pair<ChannelInfo, List<ChannelTabInfo>>> ->
            when {
                notification.isOnNext -> {
                    ...
                    eventListener.onItemCompleted(name.ifEmpty { "" })
                }
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        if (currentMode == CHANNEL_URL_MODE) channelUrl = intent.getStringExtra(KEY_VALUE)
        else {
            ...
            try {
                ...
                if (inputStreamType == null || inputStreamType == StoredFileHelper.DEFAULT_MIME) {
                    ...
                    // no extension, will fail in the extractor
                    inputStreamType = if (pointIndex == -1 || pointIndex >= name.length - 1) StoredFileHelper.DEFAULT_MIME
                    else name.substring(pointIndex + 1)
                }
            } catch (e: IOException) {
                handleError(e)
                return START_NOT_STICKY
    ...
    private fun startImport() {
        ...
        flowable.doOnNext(Consumer { subscriptionItems: List<SubscriptionItem?> ->
            ...
            .flatMap { source: List<SubscriptionItem?> -> Flowable.fromIterable(source.filterNotNull()) }
    ...
    private fun upsertBatch(): Function<List<Notification<Pair<ChannelInfo, List<ChannelTabInfo>>>>, List<SubscriptionEntity>> {
        return Function { notificationList: List<Notification<Pair<ChannelInfo, List<ChannelTabInfo>>>> ->
            ...
            for (n in notificationList) {
                if (n.isOnNext) infoList.add(n.value!!)
            }
    ...
    private fun importFromChannelUrl(): Flowable<List<SubscriptionItem?>> {
        return Flowable.fromCallable {
            val ext = Vista.getService(currentServiceId).getSubscriptionExtractor()
            ext?.fromChannelUrl(channelUrl) ?: listOf()
        }
    }
    ...
    private fun importFromInputStream(): Flowable<List<SubscriptionItem?>> {
        ...
        return Flowable.fromCallable {
            val ext = Vista.getService(currentServiceId).getSubscriptionExtractor()
            ext?.fromInputStream(inputStream!!, inputStreamType!!) ?: listOf()
        }
    }
