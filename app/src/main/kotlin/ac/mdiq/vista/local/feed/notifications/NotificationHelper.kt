package ac.mdiq.vista.local.feed.notifications
...
class NotificationHelper(val context: Context) {
    ...
    fun displayNewStreamsNotifications(data: FeedUpdateInfo) {
        ...
        val summaryBuilder = NotificationCompat.Builder(context, context.getString(R.string.streams_notification_channel_id))
        ...
        summaryBuilder.setContentIntent(
            PendingIntentCompat.getActivity(context, data.pseudoId,
                NavigationHelper.getChannelIntent(context, data.serviceId, data.url).setFlags(Intent.FLAG_ACTIVITY_NEW_TASK), 0, false))
    ...
    private fun showStreamNotifications(newStreams: List<StreamInfoItem>, serviceId: Int, channelIcon: Bitmap?) {
    ...
    private fun createStreamNotification(item: StreamInfoItem, serviceId: Int, channelIcon: Bitmap?): Notification {
        return NotificationCompat.Builder(context, context.getString(R.string.streams_notification_channel_id))
            .setSmallIcon(R.drawable.ic_newpipe_triangle_white)
            ...
            .setContentIntent(
                ...
                PendingIntentCompat.getActivity(context, item.url.hashCode(), NavigationHelper.getStreamIntent(context, serviceId, item.url, item.name), PendingIntent.FLAG_UPDATE_CURRENT, false), )
    ...
    companion object {
         ...
         * <br>
         * On Android 26 and above, notification channels are used by Vista.
        ...
        fun areNewStreamsNotificationsEnabled(context: Context): Boolean {
            return (PreferenceManager.getDefaultSharedPreferences(context)
                .getBoolean(context.getString(R.string.enable_streams_notifications), false) && areNotificationsEnabledOnDevice(context))
        }

        /**
         * Open the system's notification settings for Vista on Android Oreo (API 26) and later.
         * Open the system's app settings for Vista on previous Android versions.
         */
        fun openVoiVistaSystemNotificationSettings(context: Context) {
