...
import ac.mdiq.vista.databinding.DownloadDialogBinding
...
import ac.mdiq.vista.settings.VistaSettings.getDir
import ac.mdiq.vista.settings.VistaSettings.useStorageAccessFramework
import ac.mdiq.vista.giga.io.NoFileManagerSafeGuard.launchSafe
import ac.mdiq.vista.giga.io.StoredDirectoryHelper
import ac.mdiq.vista.giga.io.StoredDirectoryHelper.Companion.getPicker
...
import ac.mdiq.vista.util.FilePickerActivityHelper.Companion.isOwnFileUri
import ac.mdiq.vista.util.FilenameUtils.createFilename
import ac.mdiq.vista.util.ListHelper.getDefaultAudioFormat
import ac.mdiq.vista.util.ListHelper.getDefaultAudioTrackGroup
import ac.mdiq.vista.util.ListHelper.getDefaultResolutionIndex
import ac.mdiq.vista.util.ListHelper.getGroupedAudioStreams
import ac.mdiq.vista.util.ListHelper.getSortedStreamVideosList
import ac.mdiq.vista.util.ListHelper.getStreamsOfSpecifiedDelivery
...
import ac.mdiq.vista.ui.adapter.StreamItemAdapter.StreamInfoWrapper.Companion.empty
import ac.mdiq.vista.ui.adapter.StreamItemAdapter.StreamInfoWrapper.Companion.fetchMoreInfoForWrapper
import ac.mdiq.vista.ui.util.ThemeHelper.getDialogTheme
import ac.mdiq.vista.giga.get.MissionRecoveryInfo
import ac.mdiq.vista.giga.Postprocessing
import ac.mdiq.vista.giga.service.DownloadManager
import ac.mdiq.vista.giga.service.DownloadManager.MissionState
import ac.mdiq.vista.giga.service.DownloadManagerService
import ac.mdiq.vista.giga.service.DownloadManagerService.Companion.startMission
import ac.mdiq.vista.giga.service.DownloadManagerService.DownloadManagerBinder
...
class DownloadDialog : DialogFragment, RadioGroup.OnCheckedChangeListener, AdapterView.OnItemSelectedListener {
    ...
    private val requestDownloadSaveAsLauncher = registerForActivityResult(ActivityResultContracts.StartActivityForResult()) {
        result: ActivityResult -> this.requestDownloadSaveAsResult(result)
    }
    private val requestDownloadPickAudioFolderLauncher = registerForActivityResult(ActivityResultContracts.StartActivityForResult()) {
        result: ActivityResult -> this.requestDownloadPickAudioFolderResult(result)
    }
    private val requestDownloadPickVideoFolderLauncher = registerForActivityResult(ActivityResultContracts.StartActivityForResult()) {
        result: ActivityResult -> this.requestDownloadPickVideoFolderResult(result)
    }
    ...
    private val nameEditText: String
        get() {
            ...
            return createFilename(requireContext(), str.ifEmpty { currentInfo!!.name })
        }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (wrappedAudioTracks != null) this.audioTrackAdapter = AudioTrackAdapter(wrappedAudioTracks!!)
    ...
    private fun fetchStreamsSize() {
        disposables.clear()
        disposables.add(fetchMoreInfoForWrapper(wrappedVideoStreams!!).subscribe({
        ...
        disposables.add(fetchMoreInfoForWrapper(wrappedAudioStreams).subscribe({
        ...
        disposables.add(fetchMoreInfoForWrapper(wrappedSubtitleStreams!!).subscribe({
    ...
    private fun getSubtitleIndexBy(streams: List<SubtitlesStream>): Int {
        val preferredLocalization = Vista.getPreferredLocalization()

        var candidate = 0
        for (i in streams.indices) {
            val streamLocale = streams[i].locale

            val languageEquals = streamLocale.language.isNotEmpty() && preferredLocalization.languageCode.isNotEmpty()
                    && (streamLocale.language == Locale(preferredLocalization.languageCode).language)
            val countryEquals = streamLocale.country.isNotEmpty() && streamLocale.country == preferredLocalization.getCountryCode()
    ...
    private fun prepareSelectedDownload() {
        ...
        when (dBinding!!.videoAudioGroup.checkedRadioButtonId) {
            ...
            R.id.subtitle_button -> {
                ...
                if (format == MediaFormat.TTML) filenameTmp += MediaFormat.SRT.suffix
                else if (format != null) filenameTmp += format.suffix
            }
    ...
    private fun checkSelectedDownload(mainStorage: StoredDirectoryHelper?, targetFile: Uri?, filename: String?, mime: String?) {
        ...
        if (mainStorage == null) {
            ...
            // * save path not defined
            when (state) {
                MissionState.Pending, MissionState.Finished -> askDialog.setPositiveButton(msgBtn) { dialog: DialogInterface, _: Int ->
                    dialog.dismiss()
        ...
        askDialog.setPositiveButton(msgBtn) { dialog: DialogInterface, _: Int ->
            dialog.dismiss()
    ...
    private fun continueSelectedDownload(storage: StoredFileHelper) {
        ...
        when (dBinding!!.videoAudioGroup.checkedRadioButtonId) {
            R.id.audio_button -> {
                ...
                if (selectedStream.format == MediaFormat.M4A) psName = Postprocessing.ALGORITHM_M4A_NO_DASH
                else if (selectedStream.format == MediaFormat.WEBMA_OPUS) psName = Postprocessing.ALGORITHM_OGG_FROM_WEBM_DEMUXER
            }
            R.id.video_button -> {
                kind = 'v'
                ...
                if (secondary != null) {
                    ...
                    psName = if (selectedStream.format == MediaFormat.MPEG_4) Postprocessing.ALGORITHM_MP4_FROM_DASH_MUXER
            ...
            R.id.subtitle_button -> {
                ...
                if (selectedStream.format == MediaFormat.TTML) {
                    psName = Postprocessing.ALGORITHM_TTML_CONVERTER
                    psArgs = arrayOf(selectedStream.format!!.suffix, "false") // ignore empty frames
                }
            }
            else -> return
        }
        ...
        Logd(TAG, "continueSelectedDownload: urls: ${selectedStream.content} ${secondaryStream?.content}")
    ...
    class AudioTrackAdapter(private val tracksWrapper: AudioTracksWrapper) : BaseAdapter() {
        ...
        override fun getView(position: Int, convertView: View?, parent: ViewGroup): View {
            ...
            val view = convertView ?: LayoutInflater.from(context).inflate(R.layout.stream_quality_item, parent, false)
