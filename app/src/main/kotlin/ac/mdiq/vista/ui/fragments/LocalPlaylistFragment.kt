...
import ac.mdiq.vista.database.history.model.StreamHistoryEntry
import ac.mdiq.vista.database.playlist.PlaylistStreamEntry
...
import ac.mdiq.vista.databinding.LocalPlaylistHeaderBinding
...
import ac.mdiq.vista.ui.fragments.MainFragment.SelectedTabsPagerAdapter
...
import ac.mdiq.vista.ui.util.ThemeHelper.shouldUseGridLayout
...
class LocalPlaylistFragment : BaseLocalListFragment<List<PlaylistStreamEntry>, Void?>(), PlaylistControlViewHolder {
    ...
    override fun setTitle(title: String) {
        ...
        if (headerBinding != null) headerBinding!!.playlistTitleView.text = title
    }
     ...
     */
    fun commitChanges() {
        if (isModified != null && isModified!!.get()) saveImmediate()
    }

    override fun initListeners() {
        super.initListeners()
        headerBinding!!.playlistTitleView.setOnClickListener { createRenameDialog() }
        ...
        itemListAdapter!!.setSelectedListener(object : OnClickGesture<LocalItem> {
            ...
            override fun held(selectedItem: LocalItem) {
                if (selectedItem is PlaylistStreamEntry) showInfoItemDialog(selectedItem)
            }
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        when (item.itemId) {
            R.id.menu_item_share_playlist -> createShareConfirmationDialog()
            R.id.menu_item_rename_playlist -> createRenameDialog()
            R.id.menu_item_remove_watched -> if (!isRewritingPlaylist) AlertDialog.Builder(requireContext())
                .setMessage(R.string.remove_watched_popup_warning)
                .setTitle(R.string.remove_watched_popup_title)
                .setPositiveButton(R.string.ok) { _: DialogInterface?, _: Int -> removeWatchedStreams(false) }
                .setNeutralButton(R.string.remove_watched_popup_yes_and_partially_watched_videos) { _: DialogInterface?, _: Int ->
                    removeWatchedStreams(true) }
                .setNegativeButton(R.string.cancel) { d: DialogInterface, _: Int -> d.cancel() }
                .show()
            R.id.menu_item_remove_duplicates -> if (!isRewritingPlaylist) openRemoveDuplicatesDialog()
            ...
            .flatMapSingle { playlist: List<PlaylistStreamEntry> ->
                Single.just(playlist.stream()
                    ...
                    .map { streamEntity: StreamEntity ->
                        if (shouldSharePlaylistDetails) return@map context.getString(R.string.video_details_list_item,
                                streamEntity.title,
                                streamEntity.url)
                        else return@map streamEntity.url
                    }
                    .collect(Collectors.joining("\n")))
            }
            .observeOn(AndroidSchedulers.mainThread())
            .subscribe({ urlsText: String? ->
                shareText(context, name!!, if (shouldSharePlaylistDetails) context.getString(R.string.share_playlist_content_details, name, urlsText) else urlsText) },
                { throwable: Throwable? -> showUiErrorSnackbar(this, "Sharing playlist", throwable!!) }))
    }

    private fun removeWatchedStreams(removePartiallyWatched: Boolean) {
        ...
        val historyIdsMaybe = recordManager.streamHistorySortedById
            ...
            .map { historyList: List<StreamHistoryEntry> -> historyList.stream().map(StreamHistoryEntry::streamId).collect(Collectors.toList()) }
            ...
            .zipWith(historyIdsMaybe) { playlist: List<PlaylistStreamEntry>, historyStreamIds: List<Long> ->
                ...
                if (removePartiallyWatched) {
                    for (playlistItem in playlist) {
                        ...
                        if (indexInHistory < 0) itemsToKeep.add(playlistItem)
                    ...
                    for (i in playlist.indices) {
                        ...
                        if (indexInHistory < 0 || (streamStateEntity != null && !streamStateEntity.isFinished(duration))) itemsToKeep.add(playlistItem)
                        else if (!isThumbnailPermanent && !thumbnailVideoRemoved
                                && (playlistManager!!.getPlaylistThumbnailStreamId(playlistId!!) == playlistItem.streamEntity.uid))
                            thumbnailVideoRemoved = true
                    }
    ...
    private fun createRenameDialog() {
        ...
        AlertDialog.Builder(requireContext())
            ...
            .setPositiveButton(R.string.rename) { _: DialogInterface?, _: Int -> changePlaylistName(dialogBinding.dialogEditText.text.toString()) }
            .show()
    }
    ...
    private fun changeThumbnailStreamId(thumbnailStreamId: Long, isPermanent: Boolean) {
        ...
        val successToast = Toast.makeText(getActivity(), R.string.playlist_thumbnail_change_success, Toast.LENGTH_SHORT)
        ...
        val disposable = playlistManager!!
            ...
            .observeOn(AndroidSchedulers.mainThread())
            .subscribe({ _: Int? -> successToast.show() }, { throwable: Throwable? ->
    ...
    private fun updateThumbnailUrl() {
        ...
        val thumbnailStreamId = if (itemListAdapter!!.itemsList.isNotEmpty()) (itemListAdapter!!.itemsList[0] as PlaylistStreamEntry).streamEntity.uid
        else PlaylistEntity.DEFAULT_THUMBNAIL_ID
    ...
    private fun openRemoveDuplicatesDialog() {
        AlertDialog.Builder(requireActivity())
            ...
            .setPositiveButton(R.string.ok) { _: DialogInterface?, _: Int -> removeDuplicatesInPlaylist() }
    ...
    private fun deleteItem(item: PlaylistStreamEntry) {
        ...
        if (playlistManager!!.getPlaylistThumbnailStreamId(playlistId!!) == item.streamId) updateThumbnailUrl()
        setVideoCount(itemListAdapter!!.itemsList.size.toLong())
        saveChanges()
    }
    ...
    private val debouncedSaver: Disposable
        get() {
            ...
            return debouncedSaveSignal!!
                ...
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe({ saveImmediate() }, { throwable: Throwable? ->
    ...
    private fun saveImmediate() {
        ...
        val disposable = playlistManager!!.updateJoin(playlistId!!, streamIds)
            .observeOn(AndroidSchedulers.mainThread())
            .subscribe({ if (isModified != null) isModified!!.set(false) },
                { throwable: Throwable? -> showError(ErrorInfo(throwable!!, UserAction.REQUESTED_BOOKMARK, "Saving playlist")) })
        disposables!!.add(disposable)
    }
    ...
    private val itemTouchCallback: ItemTouchHelper.SimpleCallback
        get() {
            ...
            if (shouldUseGridLayout(requireContext())) directions = directions or (ItemTouchHelper.LEFT or ItemTouchHelper.RIGHT)
    ...
    @OptIn(UnstableApi::class)
    private fun showInfoItemDialog(item: PlaylistStreamEntry) {
        ...
        try {
            ...
            dialogBuilder
                ...
                .setAction(StreamDialogDefaultEntry.SET_AS_PLAYLIST_THUMBNAIL) { _: Fragment?, _: StreamInfoItem? ->
                    changeThumbnailStreamId(item.streamEntity.uid, true) }
                .setAction(StreamDialogDefaultEntry.DELETE) { _: Fragment?, _: StreamInfoItem? -> deleteItem(item) }
                .create()
                .show()
    ...
    private fun setInitialData(pid: Long, title: String) {
        this.playlistId = pid
        this.name = if (title.isNotEmpty()) title else ""
    }

    private fun setVideoCount(count: Long) {
        if (activity != null && headerBinding != null) headerBinding!!.playlistStreamCount.text = localizeStreamCount(requireActivity(), count)
    }
    ...
    private fun getPlayQueue(index: Int): PlayQueue {
        ...
        for (item in infoItems) {
            if (item is PlaylistStreamEntry) streamInfoItems.add(item.toStreamInfoItem())
        }
    ...
    private fun createShareConfirmationDialog() {
        AlertDialog.Builder(requireContext())
            ...
            .setCancelable(true)
            .setPositiveButton(R.string.share_playlist_with_titles) { _: DialogInterface?, _: Int ->
                sharePlaylist( /* shouldSharePlaylistDetails= */true) }
            .setNegativeButton(R.string.share_playlist_with_list) { _: DialogInterface?, _: Int ->
