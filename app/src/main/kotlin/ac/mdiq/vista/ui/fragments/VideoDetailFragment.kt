...
import ac.mdiq.vista.databinding.FragmentVideoDetailBinding
import ac.mdiq.vista.databinding.ListRadioIconItemBinding
import ac.mdiq.vista.databinding.RelatedItemsHeaderBinding
...
import ac.mdiq.vista.ui.util.ktx.slideUp
...
import ac.mdiq.vista.player.playqueue.events.PlayQueueEvent
import ac.mdiq.vista.ui.player.MainPlayerUi
import ac.mdiq.vista.ui.player.VideoPlayerUi
import ac.mdiq.vista.ui.adapter.TabAdapter
import ac.mdiq.vista.util.DependentPreferenceHelper.getResumePlaybackEnabled
import ac.mdiq.vista.util.DeviceUtils.getWindowHeight
import ac.mdiq.vista.util.DeviceUtils.isDesktopMode
import ac.mdiq.vista.util.DeviceUtils.isInMultiWindow
...
import ac.mdiq.vista.util.Localization.audioTrackName
import ac.mdiq.vista.util.Localization.getAppLocale
import ac.mdiq.vista.util.Localization.getDurationString
import ac.mdiq.vista.util.Localization.listeningCount
import ac.mdiq.vista.util.Localization.localizeUploadDate
import ac.mdiq.vista.util.Localization.localizeViewCount
import ac.mdiq.vista.util.Localization.localizeWatchingCount
import ac.mdiq.vista.util.Localization.shortCount
...
import ac.mdiq.vista.ui.util.NavigationHelper.getPlayerIntent
...
import ac.mdiq.vista.ui.util.NavigationHelper.openPlayQueue
...
import ac.mdiq.vista.ui.util.NavigationHelper.playOnExternalPlayer
import ac.mdiq.vista.ui.util.NavigationHelper.playOnPopupPlayer
import ac.mdiq.vista.util.PermissionHelper
import ac.mdiq.vista.util.PermissionHelper.checkStoragePermissions
import ac.mdiq.vista.util.PermissionHelper.isPopupEnabledElseAsk
import ac.mdiq.vista.ui.gesture.PlayButtonHelper.shouldShowHoldToAppendTip
import ac.mdiq.vista.database.stream.StreamTypeUtil.isLiveStream
import ac.mdiq.vista.ui.util.ThemeHelper.isLightThemeSelected
...
import ac.mdiq.vista.util.image.PicassoHelper.loadDetailsThumbnail
...
class VideoDetailFragment : BaseStateFragment<StreamInfo>(), BackPressable, PlayerServiceExtendedEventListener, OnKeyDownListener {
    ...
    private val isExternalPlayerEnabled: Boolean
        get() = PreferenceManager.getDefaultSharedPreferences(requireContext()).getBoolean(getString(R.string.use_external_video_player_key), false)
    ...
    private val isPlayerServiceAvailable: Boolean
        get() = playerService != null

    private val isPlayerAndPlayerServiceAvailable: Boolean
    ...
    override fun onServiceConnected(playerManager: PlayerManager?, playerService: PlayerService?, playAfterConnect: Boolean) {
        ...
        val playerUi = this.playerManager!!.UIs.get(MainPlayerUi::class.java)
        if (!this.playerManager!!.videoPlayerSelected() && !playAfterConnect) return
...
//        player?.UIs?.get(VideoPlayerUi::class.java)?.ifPresent { obj: VideoPlayerUi -> obj.binding.surfaceView.holder.setSizeFromLayout() }
    ...
    override fun onDestroy() {
        ...
        PreferenceManager.getDefaultSharedPreferences(requireActivity()).unregisterOnSharedPreferenceChangeListener(preferenceChangeListener)
...
//            bottomSheetBehavior?.setState(BottomSheetBehavior.STATE_HIDDEN)
    ...
    override fun initViews(rootView: View, savedInstanceState: Bundle?) {
        ...
        binding.detailControlsCrashThePlayer.visibility =
            if (DEBUG && PreferenceManager.getDefaultSharedPreferences(requireContext()).getBoolean(getString(R.string.show_crash_the_player_key), false)) View.VISIBLE else View.GONE
    ...
    private fun initTabs() {
        ...
        if (pageAdapter.count == 0) pageAdapter.addFragment(EmptyFragment.newInstance(true), EMPTY_TAB_TAG)
    ...
    private fun updateTabs(info: StreamInfo) {
        if (showRelatedItems) {
            if (binding.relatedItemsLayout == null)
                pageAdapter.updateItem(RELATED_TAB_TAG, RelatedItemsFragment.getInstance(info)) // phone
            else { // tablet + TV
    ...
    private fun shouldShowComments(): Boolean {
        return try {
            showComments && Vista.getService(serviceId)
                .serviceInfo
                .mediaCapabilities
    ...
    fun updateTabLayoutVisibility() {
        ...
        if (pageAdapter.count < 2 || binding.viewPager.visibility != View.VISIBLE) binding.tabLayout.visibility = View.GONE
        else {
...
//        playerIntent.putExtra(PLAYER_TYPE, "mediaPlayback")
    ...
    private fun startOnExternalPlayer(context: Context, info: StreamInfo, selectedStream: Stream) {
        ...
        disposables.add(recordManager.onViewed(info).onErrorComplete()
            .subscribe({ ignored: Long? -> }, { error: Throwable? -> Log.e(TAG, "Register view failure: ", error) }))
    }
    ...
    private fun tryAddVideoPlayerView() {
        ...
        if (isPlayerAvailable && view != null) setHeightThumbnail()
...
//                    ACTION_HIDE_MAIN_PLAYER -> bottomSheetBehavior!!.setState(BottomSheetBehavior.STATE_HIDDEN)
    ...
    override fun handleResult(info: StreamInfo) {
        ...
        if (info.subChannelName.isNotEmpty()) displayBothUploaderAndSubChannel(info)
        ...
        } else binding.detailViewCountView.visibility = View.GONE
    ...
    private fun displayUploaderAsSubChannel(info: StreamInfo) {
        ...
        } else binding.detailUploaderTextView.visibility = View.GONE
    ...
    private fun displayBothUploaderAndSubChannel(info: StreamInfo) {
        ...
        if (info.uploaderName.isNotEmpty()) subText.append(String.format(getString(R.string.video_detail_by), info.uploaderName))
    ...
    private fun setupBrightness() {
        ...
        if (!isFullscreen || bottomSheetState != BottomSheetBehavior.STATE_EXPANDED) restoreDefaultBrightness()
        else {
    ...
    private fun showExternalVideoPlaybackDialog() {
        ...
        if (videoStreamsForExternalPlayers.isEmpty()) {
            ...
            val resolutions = videoStreamsForExternalPlayers.stream()
                .map { obj: VideoStream -> obj.resolution }
    ...
    private fun updateOverlayData(overlayTitle: String?, uploader: String?, thumbnails: List<Image>) {
        binding.overlayTitleTextView.text = if (overlayTitle.isNullOrEmpty()) "" else overlayTitle
        binding.overlayChannelTextView.text = if (uploader.isNullOrEmpty()) "" else uploader
    ...
    object VideoDetailPlayerCrasher {
        ...
        private val AVAILABLE_EXCEPTION_TYPES: List<Pair<String, Supplier<ExoPlaybackException>>> = listOf(
            ...
            Pair("Renderer", Supplier {
                ExoPlaybackException.createForRenderer(Exception(DEFAULT_MSG), "Dummy renderer", 0,
                    null, C.FORMAT_HANDLED,  /*isRecoverable=*/ false, PlaybackException.ERROR_CODE_DECODING_FAILED)
            }),
        ...
        private fun tryCrashPlayerWith(playerManager: PlayerManager, exception: ExoPlaybackException) {
            ...
            try {
                ...
                Log.e(TAG, "Run into an exception while crashing the player:", exPlayer)
            }
        }
    }
...
//            TODO: get rid of the !!
    ...
    class DescriptionFragment : BaseDescriptionFragment {
        ...
        override fun setupMetadata(inflater: LayoutInflater?, layout: LinearLayout?) {
            ...
            else binding!!.detailUploadDateView.visibility = View.GONE
            ...
            if (streamInfo!!.languageInfo != null)
                addMetadataItem(inflater, layout, false, R.string.metadata_language, streamInfo!!.languageInfo?.getDisplayLanguage(
                    getAppLocale(requireContext())))
        ...
        private fun addPrivacyMetadataItem(inflater: LayoutInflater?, layout: LinearLayout?) {
            if (streamInfo!!.privacy != null) {
                ...
                if (contentRes != 0) addMetadataItem(inflater, layout!!, false, R.string.metadata_privacy, getString(contentRes))
            }
        }
    }
    ...
    class RelatedItemsFragment : BaseListInfoFragment<InfoItem, RelatedItemsFragment.RelatedItemsInfo>(UserAction.REQUESTED_STREAM), OnSharedPreferenceChangeListener {
        ...
        override val listHeaderSupplier: Supplier<View>?
            get() {
                if (relatedItemsInfo == null) return null
        ...
        override fun onRestoreInstanceState(savedInstanceState: Bundle) {
            ...
            val serializable = savedInstanceState.getSerializable(INFO_KEY)
        ...
        class RelatedItemsInfo(info: StreamInfo)
            : ListInfo<InfoItem>(info.serviceId, ListLinkHandler(info.originalUrl, info.url, info.id, emptyList(), ""), info.name) {
            /**
