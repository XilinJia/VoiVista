...
import ac.mdiq.vista.databinding.FragmentChannelBinding
...
import ac.mdiq.vista.local.feed.notifications.NotificationHelper.Companion.areNewStreamsNotificationsEnabled
import ac.mdiq.vista.manager.SubscriptionManager
import ac.mdiq.vista.player.playqueue.ChannelTabPlayQueue
...
import ac.mdiq.vista.ui.util.ShareUtils.openUrlInApp
...
import ac.mdiq.vista.ui.util.ktx.animateBackgroundColor
import ac.mdiq.vista.ui.util.ktx.animateTextColor
import ac.mdiq.vista.util.ChannelTabHelper.getTranslationKey
import ac.mdiq.vista.util.ChannelTabHelper.isStreamsTab
import ac.mdiq.vista.util.ChannelTabHelper.showChannelTab
...
import ac.mdiq.vista.util.Localization.localizeNumber
...
import ac.mdiq.vista.util.image.ImageStrategy.imageListToDbUrl
...
import ac.mdiq.vista.util.image.PicassoHelper.loadBanner
import java.util.*
...
class ChannelFragment : BaseStateFragment<ChannelInfo>(), WriteRead {
    ...
    private fun setInitialData(sid: Int, u: String, title: String) {
        Logd(TAG, "setInitialData: url: $u")
        this.serviceId = sid
        this.url = u
        this.name = title.ifEmpty { "" }
    }
    ...
    override fun initListeners() {
        ...
        val openSubChannel = View.OnClickListener { _: View? ->
            if (!currentInfo!!.parentChannelUrl.isNullOrEmpty()) {
                try {
                    openChannelFragment(fM!!, currentInfo!!.serviceId, currentInfo!!.parentChannelUrl, currentInfo!!.parentChannelName?:"")
                } catch (e: Exception) {
    ...
    private fun monitorSubscription(info: ChannelInfo) {
        ...
        disposables.add(observable
            ...
            .observeOn(AndroidSchedulers.mainThread())
            .subscribe({ isEmpty: Boolean? -> if (!isEmpty!!) showNotifySnackbar() }, onError))
    }
    ...
    private fun runWorker(forceLoad: Boolean) {
        if (url != null) currentWorker = getChannelInfo(serviceId, url!!, forceLoad)
            .subscribeOn(Schedulers.io())
            .observeOn(AndroidSchedulers.mainThread())
            .subscribe({ result: ChannelInfo ->
                isLoading.set(false)
                handleResult(result)
            }, { throwable: Throwable? -> showError(ErrorInfo(throwable!!, UserAction.REQUESTED_CHANNEL, url ?: "No URL", serviceId)) })
    }
    ...
    override fun handleResult(result: ChannelInfo) {
        ...
        if (!currentInfo!!.parentChannelName.isNullOrEmpty()) {
        ...
        if (menuRssButton != null) menuRssButton!!.setVisible(!result.feedUrl.isNullOrEmpty())
    ...
    class ChannelAboutFragment : BaseDescriptionFragment() {
        ...
        override fun setupMetadata(inflater: LayoutInflater?, layout: LinearLayout?) {
            ...
            val context = context
            if (channelInfo!!.subscriberCount != ChannelExtractor.UNKNOWN_SUBSCRIBER_COUNT)
        ...
        override fun handleResult(result: ChannelTabInfo) {
            ...
            if (tabHandler is ReadyChannelTabListLinkHandler) {
                try {
                    ...
                    val channelTabLHFactory = result.service.getChannelTabLHFactory()
