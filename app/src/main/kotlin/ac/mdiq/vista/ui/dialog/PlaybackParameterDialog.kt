...
import ac.mdiq.vista.databinding.DialogPlaybackParameterBinding
import ac.mdiq.vista.ui.util.ktx.animateRotation
import ac.mdiq.vista.ui.dialog.PlaybackParameterDialog.PlayerSemitoneHelper.formatPitchSemitones
import ac.mdiq.vista.ui.dialog.PlaybackParameterDialog.PlayerSemitoneHelper.percentToSemitones
import ac.mdiq.vista.ui.dialog.PlaybackParameterDialog.PlayerSemitoneHelper.semitonesToPercent
import ac.mdiq.vista.player.helper.PlayerHelper.formatPitch
import ac.mdiq.vista.player.helper.PlayerHelper.formatSpeed
...
import ac.mdiq.vista.ui.gesture.SimpleOnSeekBarChangeListener
import ac.mdiq.vista.ui.gesture.SliderStrategy
import ac.mdiq.vista.ui.gesture.SliderStrategy.Quadratic
import ac.mdiq.vista.ui.util.ThemeHelper.resolveDrawable
import java.util.*
...
class PlaybackParameterDialog : DialogFragment() {
    ...
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        ...
        val dialogBuilder = AlertDialog.Builder(requireActivity())
            ...
            .setNeutralButton(R.string.playback_reset) { _: DialogInterface?, _: Int ->
                setAndUpdateTempo(DEFAULT_TEMPO)
                setAndUpdatePitch(DEFAULT_PITCH_PERCENT)
                setAndUpdateSkipSilence(DEFAULT_SKIP_SILENCE)
                updateCallback()
            }
            .setPositiveButton(R.string.ok) { _: DialogInterface?, _: Int -> updateCallback() }
    ...
    private fun initUI() {
        // Tempo
        setText(binding!!.tempoMinimumText, { obj: Double -> formatSpeed(obj) }, MIN_PITCH_OR_SPEED)
        setText(binding!!.tempoMaximumText, { obj: Double -> formatSpeed(obj) }, MAX_PITCH_OR_SPEED)
        ...
        binding!!.tempoSeekbar.setOnSeekBarChangeListener(
            getTempoOrPitchSeekbarChangeListener(QUADRATIC_STRATEGY) { newTempo: Double -> this.onTempoSliderUpdated(newTempo) })

        registerOnStepClickListener(binding!!.tempoStepDown, { tempo }, -1.0, { newTempo: Double -> this.onTempoSliderUpdated(newTempo) })
        registerOnStepClickListener(binding!!.tempoStepUp, { tempo }, 1.0, { newTempo: Double -> this.onTempoSliderUpdated(newTempo) })

        // Pitch
        binding!!.pitchToogleControlModes.setOnClickListener {
            val isCurrentlyVisible = binding!!.pitchControlModeTabs.visibility == View.GONE
            binding!!.pitchControlModeTabs.visibility = if (isCurrentlyVisible) View.VISIBLE else View.GONE
            binding!!.pitchToogleControlModes.animateRotation(VideoPlayerUi.DEFAULT_CONTROLS_DURATION, if (isCurrentlyVisible) 180 else 0)
        }

        pitchControlModeComponentMappings.forEach { (semitones: Boolean, textView: TextView) ->
            this.setupPitchControlModeTextView(semitones, textView)
        }

        // Initialization is done at the end

        // Pitch - Percent
        setText(binding!!.pitchPercentMinimumText, { obj: Double -> formatSpeed(obj) }, MIN_PITCH_OR_SPEED)
        setText(binding!!.pitchPercentMaximumText, { obj: Double -> formatSpeed(obj) }, MAX_PITCH_OR_SPEED)
        ...
        binding!!.pitchPercentSeekbar.setOnSeekBarChangeListener(
            getTempoOrPitchSeekbarChangeListener(QUADRATIC_STRATEGY) { newPitch: Double -> this.onPitchPercentSliderUpdated(newPitch) })

        registerOnStepClickListener(binding!!.pitchPercentStepDown, { pitchPercent }, -1.0,
            { newPitch: Double -> this.onPitchPercentSliderUpdated(newPitch) })
        registerOnStepClickListener(binding!!.pitchPercentStepUp, { pitchPercent }, 1.0,
            { newPitch: Double -> this.onPitchPercentSliderUpdated(newPitch) })

        // Pitch - Semitone
        binding!!.pitchSemitoneSeekbar.setOnSeekBarChangeListener(
            getTempoOrPitchSeekbarChangeListener(SEMITONE_STRATEGY) { newPitch: Double -> this.onPitchPercentSliderUpdated(newPitch) })

        registerOnSemitoneStepClickListener(binding!!.pitchSemitoneStepDown, -1) { newPitch: Double -> this.onPitchPercentSliderUpdated(newPitch) }
        registerOnSemitoneStepClickListener(binding!!.pitchSemitoneStepUp, 1) { newPitch: Double -> this.onPitchPercentSliderUpdated(newPitch) }

        // Steps
        stepSizeComponentMappings.forEach { (stepSizeValue: Double, textView: TextView) -> this.setupStepTextView(stepSizeValue, textView) }
        // Initialize UI
        setStepSizeToUI(currentStepSize)

        // Bottom controls
        bindCheckboxWithBoolPref(binding!!.unhookCheckbox, R.string.playback_unhook_key, true) { isChecked: Boolean? ->
            // when unchecked, slide back to the minimum of current tempo or pitch
            if (!isChecked!!) ensureHookIsValidAndUpdateCallBack()
        }

        setAndUpdateSkipSilence(skipSilence)
        binding!!.skipSilenceCheckbox.setOnCheckedChangeListener { _: CompoundButton?, isChecked: Boolean ->
    ...
    // -- General formatting --
    private fun setText(textView: TextView, formatter: DoubleFunction<String>, value: Double) {
        Objects.requireNonNull(textView).text = formatter.apply(value)
    }

    // -- Steps -- /* -1 for step down, +1 for step up */
    private fun registerOnStepClickListener(stepTextView: TextView, currentValueSupplier: DoubleSupplier, direction: Double,  newValueConsumer: DoubleConsumer) {
        stepTextView.setOnClickListener {
            newValueConsumer.accept(currentValueSupplier.asDouble + 1 * currentStepSize * direction)
            updateCallback()
        }
    }

    /* -1 for step down, +1 for step up */
    private fun registerOnSemitoneStepClickListener(stepTextView: TextView, direction: Int,  newValueConsumer: DoubleConsumer) {
        stepTextView.setOnClickListener {
    ...
    // -- Pitch --
    private fun setupPitchControlModeTextView(semitones: Boolean, textView: TextView) {
    ...
    private fun changePitchControlMode(semitones: Boolean) {
        ...
        val pitchCtrlModeComponentMapping = pitchControlModeComponentMappings
        pitchCtrlModeComponentMapping.forEach { (_: Boolean?, textView: TextView) ->
        ...
        if (textView != null) textView.background = LayerDrawable(arrayOf(resolveDrawable(requireContext(), R.attr.dashed_border),
    ...
    private val isCurrentPitchControlModeSemitone: Boolean
        get() = PreferenceManager.getDefaultSharedPreferences(requireContext())
            .getBoolean(getString(R.string.playback_adjust_by_semitones_key), PITCH_CTRL_MODE_PERCENT)

    // -- Steps (Set) --
    private fun setupStepTextView(stepSizeValue: Double, textView: TextView) {
    ...
    private fun setStepSizeToUI(newStepSize: Double) {
        ...
        stepSiteComponentMapping.forEach { (_: Double?, textView: TextView) ->
            textView.background = resolveDrawable(requireContext(), R.attr.selectableItemBackground)
        }
    ...
    private val currentStepSize: Double
        get() = PreferenceManager.getDefaultSharedPreferences(requireContext()).getFloat(getString(R.string.adjustment_step_key), DEFAULT_STEP.toFloat()).toDouble()
    ...
    private fun bindCheckboxWithBoolPref(checkBox: CheckBox, @StringRes resId: Int, defaultValue: Boolean, onInitialValueOrValueChange: Consumer<Boolean>) {
    ...
    private fun getTempoOrPitchSeekbarChangeListener(sliderStrategy: SliderStrategy, newValueConsumer: DoubleConsumer): OnSeekBarChangeListener {
    ...
    private fun onTempoSliderUpdated(newTempo: Double) {
        if (!binding!!.unhookCheckbox.isChecked) setSliders(newTempo)
        else setAndUpdateTempo(newTempo)
    }

    private fun onPitchPercentSliderUpdated(newPitch: Double) {
        if (!binding!!.unhookCheckbox.isChecked) setSliders(newPitch)
        else setAndUpdatePitch(newPitch)
    }
    ...
    private fun setAndUpdatePitch(newPitch: Double) {
        ...
        setText(binding!!.pitchPercentCurrentText, { obj: Double -> formatSpeed(obj) }, pitchPercent)
    ...
    private fun calcValidPitch(newPitch: Double): Double {
        ...
        if (!isCurrentPitchControlModeSemitone) return calcPitch
        return semitonesToPercent(percentToSemitones(calcPitch))
    }
    ...
    object PlayerSemitoneHelper {
        private const val SEMITONE_COUNT: Int = 12
    ...
    companion object {
        ...
        private val QUADRATIC_STRATEGY: SliderStrategy = Quadratic(MIN_PITCH_OR_SPEED, MAX_PITCH_OR_SPEED, 1.00, 10000)
        ...
        fun newInstance(playbackTempo: Double, playbackPitch: Double, playbackSkipSilence: Boolean, callback: Callback?): PlaybackParameterDialog {
