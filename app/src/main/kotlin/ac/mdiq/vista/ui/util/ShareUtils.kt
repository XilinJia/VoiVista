...
object ShareUtils {
    ...
    fun installApp(context: Context, packageId: String) {
        ...
        if (!tryOpenIntentInApp(context, marketSchemeIntent)) openUrlInApp(context, "https://play.google.com/store/apps/details?id=$packageId")
    }
    ...
    fun openUrlInBrowser(context: Context, url: String?) {
        ...
        defaultBrowserInfo = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU)
            context.packageManager.resolveActivity(browserIntent, PackageManager.ResolveInfoFlags.of(PackageManager.MATCH_DEFAULT_ONLY.toLong()))
        else context.packageManager.resolveActivity(browserIntent, PackageManager.MATCH_DEFAULT_ONLY)
        ...
        if (defaultBrowserPackage == "android") openAppChooser(context, intent, true)
        else {
            try {
    ...
    fun openUrlInApp(context: Context, url: String?) {
        openIntentInApp(context, Intent(Intent.ACTION_VIEW, Uri.parse(url)).setFlags(Intent.FLAG_ACTIVITY_NEW_TASK))
    }
    ...
    fun openIntentInApp(context: Context, intent: Intent) {
        if (!tryOpenIntentInApp(context, intent)) Toast.makeText(context, R.string.no_app_to_open_intent, Toast.LENGTH_LONG).show()
    }
     ...
     */
    private fun openAppChooser(context: Context, intent: Intent, setTitleChooser: Boolean) {
        val chooserIntent = Intent(Intent.ACTION_CHOOSER)
        chooserIntent.putExtra(Intent.EXTRA_INTENT, intent)
        chooserIntent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
        if (setTitleChooser) chooserIntent.putExtra(Intent.EXTRA_TITLE, context.getString(R.string.open_with))
        ...
        if (permFlags != 0) {
            ...
            if (targetClipData == null && intent.data != null) {
                ...
                val mimeTypes = if (intent.type != null) arrayOf(intent.type) else arrayOf()
     ...
     */
    @JvmOverloads
    fun shareText(context: Context, title: String, content: String?, imagePreviewUrl: String? = "") {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q && !imagePreviewUrl.isNullOrEmpty() && ImageStrategy.shouldLoadImages()) {
     ...
     */
    fun shareText(context: Context, title: String, content: String?, images: List<Image>?) {
    ...
    fun copyToClipboard(context: Context, text: String?) {
        ...
        try {
            ...
            if (Build.VERSION.SDK_INT < 33) Toast.makeText(context, R.string.msg_copied, Toast.LENGTH_SHORT).show()
        } catch (e: Exception) {
     ...
     */
    private fun generateClipDataForImagePreview(context: Context, thumbnailUrl: String): ClipData? {
        try {
            ...
            val thumbnailPreviewFile = File("$appFolder/android_share_sheet_image_preview.jpg")
