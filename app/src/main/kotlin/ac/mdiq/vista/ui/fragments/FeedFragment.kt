 ...
 * FeedFragment.kt is part of Vista
 *
 * License: GPL-3.0+
...
import ac.mdiq.vista.databinding.FragmentFeedBinding
import ac.mdiq.vista.util.error.ErrorInfo
import ac.mdiq.vista.util.error.ErrorUtil
...
import ac.mdiq.vista.local.feed.FeedState
import ac.mdiq.vista.local.feed.StreamItem
...
import ac.mdiq.vista.ui.util.ThemeHelper.getGridSpanCountStreams
...
import ac.mdiq.vista.ui.viewmodel.FeedViewModel
...
class FeedFragment : BaseStateFragment<FeedState>() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        onSettingsChangeListener = SharedPreferences.OnSharedPreferenceChangeListener { _, key ->
            if (getString(R.string.list_view_mode_key) == key) updateListViewModeOnResume = true
        }
            ...
            R.id.menu_item_feed_help -> {
                ...
                AlertDialog.Builder(requireContext())
                    ...
                    .setNeutralButton(enableDisableButtonText) { _, _ ->
                        sharedPreferences.edit { putBoolean(getString(R.string.feed_use_dedicated_fetch_method_key), !usingDedicatedMethod) }
                    }
    ...
    private fun showStreamVisibilityDialog() {
        ...
        AlertDialog.Builder(requireContext())
            ...
            .setMultiChoiceItems(dialogItems, checkedDialogItems) { _, which, isChecked -> checkedDialogItems[which] = isChecked }
    ...
    override fun handleResult(result: FeedState) {
        Logd(TAG, "handleResult result: $result")
        when (result) {
    ...
    private fun handleLoadedState(loadedState: FeedState.LoadedState) {
        ...
        Logd(TAG, "handleLoadedState loadedState.items: ${loadedState.items.size}")
        ...
        groupAdapter.updateAsync(loadedState.items, false) {
            oldOldestSubscriptionUpdate?.run { highlightNewItemsAfter(oldOldestSubscriptionUpdate) }
        }
    ...
    private fun handleItemsErrors(errors: List<Throwable>) {
        errors.forEachIndexed { i, t ->
            if (t is FeedLoadService.RequestException && t.cause is ContentNotAvailableException) {
                disposables.add(Single.fromCallable {
                    VoiVistaDatabase.getInstance(requireContext()).subscriptionDAO().getSubscription(t.subscriptionId) }
        ...
        if (errors.isNotEmpty()) ErrorUtil.showSnackbar(this, ErrorInfo(errors, UserAction.REQUESTED_FEED, ""))
    }
                ...
                when {
                    isFastFeedModeEnabled -> {
                        ...
                        builder.setNeutralButton(R.string.feed_use_dedicated_fetch_method_disable_button) { _, _ ->
                            sharedPreferences.edit { putBoolean(getString(R.string.feed_use_dedicated_fetch_method_key), false) }
                        }
                    }
                    !cause.message.isNullOrEmpty() -> message += "\n" + cause.message
                }
            }
        }
    ...
    private fun highlightNewItemsAfter(updateTime: OffsetDateTime) {
        ...
        for (i in 0 until groupAdapter.itemCount) {
            ...
            var backgroundSupplier = { ctx: Context -> resolveDrawable(ctx, R.attr.selectableItemBackground) }
            if (doCheck) {
                ...
                if (item.streamWithState.stream.uploadDate?.isAfter(updateTime) != false) {
                    ...
                    backgroundSupplier = { ctx: Context ->
                        ...
                        LayerDrawable(arrayOf(resolveDrawable(ctx, R.attr.dashed_border), resolveDrawable(ctx, R.attr.selectableItemBackground)))
                    }
                } else {
    ...
    private fun showNewItemsLoaded() {
        ...
        tryGetNewItemsLoadedButton()
            ...
            ?.slideUp(250L, delay = 100,
                execOnEnd = { context?.let { if (DeviceUtils.hasAnimationsAnimatorDurationEnabled(it)) hideNewItemsLoaded(true, 10000) } },
            )
    }
    ...
    private fun hideNewItemsLoaded(animate: Boolean, delay: Long = 0) {
        ...
        if (animate)
            tryGetNewItemsLoadedButton()?.animate(false, 200, delay = delay, execOnEnd = { tryGetNewItemsLoadedButton()?.isVisible = false })
    ...
    override fun reloadContent() {
        ...
        getActivity()?.startService(Intent(requireContext(), FeedLoadService::class.java).apply { putExtra(FeedLoadService.EXTRA_GROUP_ID, groupId) })
        listState = null
    }
