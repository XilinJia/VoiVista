...
class StreamItemAdapter<T : Stream, U : Stream>(
    ...
    private fun getCustomView(position: Int, view: View?, parent: ViewGroup, isDropdownItem: Boolean): View {
        ...
        when (stream) {
            is VideoStream -> {
                ...
                qualityString = videoStream.resolution
                ...
                if (hasAnyVideoOnlyStreamWithNoSecondaryStream) {
                    if (videoStream.isVideoOnly) {
                        ...
                        woSoundIconVisibility = if (allSecondary[position] != null) if (isDropdownItem) View.INVISIBLE else View.GONE else View.VISIBLE
                    } else if (isDropdownItem) woSoundIconVisibility = View.INVISIBLE
                }
            }
            is AudioStream -> {
                val audioStream = (stream as AudioStream)
                qualityString = if (audioStream.averageBitrate > 0) audioStream.averageBitrate.toString() + "kbps"
                else context.getString(R.string.unknown_quality)
            }
            is SubtitlesStream -> {
                qualityString = (stream as SubtitlesStream).displayLanguageName
                if ((stream as SubtitlesStream).isAutoGenerated) qualityString += " (" + context.getString(R.string.caption_auto_generated) + ")"
            }
            else -> qualityString = mediaFormat?.suffix ?: context.getString(R.string.unknown_quality)
        }

        if (streamsWrapper.getSizeInBytes(position) > 0) {
            val secondary = allSecondary[position]
            if (secondary != null) {
                val size = (secondary.sizeInBytes + streamsWrapper.getSizeInBytes(position))
                sizeView.text = Utility.formatBytes(size)
            } else sizeView.text = streamsWrapper.getFormattedSize(position)
            sizeView.visibility = View.VISIBLE
        } else sizeView.visibility = View.GONE

        if (stream is SubtitlesStream) formatNameView.text = (stream as SubtitlesStream).languageTag
        else {
            when (mediaFormat) {
                null -> formatNameView.text = context.getString(R.string.unknown_format)
                // noinspection AndroidLintSetTextI18n
                MediaFormat.WEBMA_OPUS -> formatNameView.text = "opus"
                else -> formatNameView.text = mediaFormat.getName()
            }
        }
    ...
    private fun checkHasAnyVideoOnlyStreamWithNoSecondaryStream(): Boolean {
        for (i in streamsWrapper.streamsList.indices) {
            ...
            if (stream is VideoStream) {
                ...
                if (videoOnly && allSecondary[i] == null) return true
            }
        }
    ...
    class StreamInfoWrapper<T : Stream>(streamList: List<T>, context: Context?) : Serializable {
        @JvmField
        ...
        private fun formatSize(size: Long): String {
            if (size > -1) return Utility.formatBytes(size)
            return unknownSize
        }
        ...
        companion object {
            private val EMPTY = StreamInfoWrapper(emptyList(), null)
            ...
            fun <X : Stream> fetchMoreInfoForWrapper(streamsWrapper: StreamInfoWrapper<X>): Single<Boolean> {
                val fetchAndSet = Callable {
                    ...
                    for (stream in streamsWrapper.streamsList) {
                        ...
                        if (!changeSize && !changeFormat) continue
                        ...
                        if (changeFormat) hasChanged = (retrieveMediaFormat(stream, streamsWrapper, response) || hasChanged)
                    }
                    hasChanged
                }
            ...
            @VisibleForTesting
            fun <X : Stream> retrieveMediaFormat(stream: X, streamsWrapper: StreamInfoWrapper<X>, response: Response): Boolean {
            ...
            @VisibleForTesting
            fun <X : Stream> retrieveMediaFormatFromFileTypeHeaders(stream: X, streamsWrapper: StreamInfoWrapper<X>, response: Response): Boolean {
                // try to use additional headers from CDNs or servers,
                // e.g. x-amz-meta-file-type (e.g. for SoundCloud)
                val keys = response.responseHeaders().keys.stream().filter { k: String -> k.endsWith("file-type") }.collect(Collectors.toList())
                if (keys.isNotEmpty()) {
                    for (key in keys) {
                        val suffix = response.getHeader(key) ?: continue
            ...
            @VisibleForTesting
            fun <X : Stream> retrieveMediaFormatFromContentDispositionHeader(stream: X, streamsWrapper: StreamInfoWrapper<X>, response: Response): Boolean {
                ...
                try {
                    ...
                    for (part_ in parts) {
                        ...
                        val fileName = when {
                            ...
                            part.startsWith("filename=") -> Utils.decodeUrlUtf8(part.substring(9))
                            part.startsWith("filename*=") -> Utils.decodeUrlUtf8(part.substring(10))
                            else -> continue
                        }
                        ...
                        if (suffix.endsWith("\"") || suffix.endsWith("'")) suffix = suffix.substring(0, suffix.length - 1)
            ...
            @VisibleForTesting
            fun <X : Stream> retrieveMediaFormatFromContentTypeHeader(stream: X, streamsWrapper: StreamInfoWrapper<X>, response: Response): Boolean {
                ...
                for (format in MediaFormat.getAllFromMimeType(contentTypeHeader)) {
                    if (foundFormat == null) foundFormat = format
                    else if (foundFormat.id != format.id) return false
                }
                if (foundFormat != null) {
