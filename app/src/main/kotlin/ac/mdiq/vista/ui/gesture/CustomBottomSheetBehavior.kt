...
class CustomBottomSheetBehavior(context: Context, attrs: AttributeSet?) : BottomSheetBehavior<FrameLayout?>(context, attrs) {
    ...
    override fun onInterceptTouchEvent(parent: CoordinatorLayout, child: FrameLayout, event: MotionEvent): Boolean {
        ...
        if (event.action == MotionEvent.ACTION_CANCEL || event.action == MotionEvent.ACTION_UP) skippingInterception = false

        // Found that user still swiping, continue following
        if (skippingInterception || state == STATE_SETTLING) return false
        ...
        if (event.pointerCount == 2) return super.onInterceptTouchEvent(parent, child, event)

        // Don't need to do anything if bottomSheet isn't expanded
        if (state == STATE_EXPANDED && event.action == MotionEvent.ACTION_DOWN) {
            ...
            for (element in skipInterceptionOfElements) {
                ...
                if (view != null) {
                    ...
                    if (visible && globalRect.contains(event.rawX.toInt(), event.rawY.toInt())) {
                        // Makes bottom part of the player draggable in portrait when
                        // playbackControlRoot is hidden
                        if (element == R.id.bottomControls && child.findViewById<View>(R.id.playbackControlRoot).visibility != View.VISIBLE)
