 ...
 * VoiVistaRecyclerView.kt is part of Vista.
 *
...
class VistaRecyclerView : RecyclerView {
    ...
    private fun tryFocusFinder(direction: Int): Boolean {
        ...
        while (parent !== root) {
            ...
            if (candidate != null && candidate.requestFocus(direction)) return true
    ...
    private fun arrowScroll(direction: Int): Boolean {
        when (direction) {
            ...
            FOCUS_LEFT -> {
                if (!canScrollHorizontally(-1)) return false
                scrollBy(-100, 0)
            }
            FOCUS_RIGHT -> {
                if (!canScrollHorizontally(-1)) return false
    ...
    companion object {
        private const val TAG = "VoiVistaRecyclerView"
    }
}
