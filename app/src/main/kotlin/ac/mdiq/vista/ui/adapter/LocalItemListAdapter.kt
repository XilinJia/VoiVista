package ac.mdiq.vista.ui.adapter
...
import ac.mdiq.vista.util.Localization.getPreferredLocale
...
//        Logd(TAG, "onBindViewHolder() called with: holder = [${holder.javaClass.simpleName}], position = [$position]")
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int, payloads: List<Any>) {
        if (payloads.isNotEmpty() && holder is LocalItemHolder) {
    ...
    open class RemotePlaylistItemHolder : PlaylistItemHolder {
        ...
        override fun updateFromItem(item: LocalItem?, historyRecordManager: HistoryRecordManager?, dateTimeFormatter: DateTimeFormatter?) {
            ...
            if (item.uploader.isNotEmpty()) itemUploaderView.text =
                concatenateStrings(item.uploader, getNameOfServiceById(item.serviceId))
    ...
    open class LocalStatisticStreamItemHolder internal constructor(infoItemBuilder: LocalItemBuilder?, layoutId: Int, parent: ViewGroup?)
        ...
        override fun updateFromItem(item: LocalItem?, historyRecordManager: HistoryRecordManager?, dateTimeFormatter: DateTimeFormatter?) {
            ...
            if (itemAdditionalDetails != null) itemAdditionalDetails.text =
        ...
        override fun updateState(localItem: LocalItem?, historyRecordManager: HistoryRecordManager?) {
            ...
            if (getPositionsInListsEnabled(itemProgressView.context) && localItem.progressMillis > 0 && localItem.streamEntity.duration > 0) {
                itemProgressView.max = localItem.streamEntity.duration.toInt()
                if (itemProgressView.visibility == View.VISIBLE)
                    itemProgressView.setProgressAnimated(TimeUnit.MILLISECONDS.toSeconds(localItem.progressMillis).toInt())
                else {
                    itemProgressView.progress = TimeUnit.MILLISECONDS.toSeconds(localItem.progressMillis).toInt()
    ...
    abstract class PlaylistItemHolder(infoItemBuilder: LocalItemBuilder?, layoutId: Int, parent: ViewGroup?)
        ...
        override fun updateFromItem(item: LocalItem?, historyRecordManager: HistoryRecordManager?, dateTimeFormatter: DateTimeFormatter?) {
            itemView.setOnClickListener {
                if (item != null) itemBuilder.onItemSelectedListener?.selected(item)
            }
            itemView.isLongClickable = true
            itemView.setOnLongClickListener {
                if (item != null) itemBuilder.onItemSelectedListener?.held(item)
                true
            }
        }
    }
    ...
    open class LocalPlaylistStreamItemHolder internal constructor(infoItemBuilder: LocalItemBuilder?, layoutId: Int, parent: ViewGroup?)
        ...
        override fun updateFromItem(item: LocalItem?, historyRecordManager: HistoryRecordManager?, dateTimeFormatter: DateTimeFormatter?) {
            ...
            itemVideoTitleView.text = item.streamEntity.title
            itemAdditionalDetailsView.text = concatenateStrings(item.streamEntity.uploader, getNameOfServiceById(
                item.streamEntity.serviceId))
