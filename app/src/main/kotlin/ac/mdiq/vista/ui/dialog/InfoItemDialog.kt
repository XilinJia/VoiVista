 ...
 */
class InfoItemDialog private constructor(activity: Activity, fragment: Fragment, info: StreamInfoItem, entries: List<StreamDialogEntry>) {
    /**
    ...
    init {
        ...
        } else detailsView.visibility = View.GONE

        // Get the entry's descriptions which are displayed in the dialog
        val items = entries.stream()
            .map { entry: StreamDialogEntry -> entry.getString(activity) }
...
//            .toArray<String> { _Dummy_.__Array__() }
        ...
        val action = DialogInterface.OnClickListener { _: DialogInterface?, index: Int ->
            entries[index].action.onClick(fragment, info)
        }

        dialog = AlertDialog.Builder(activity).setCustomTitle(bannerView).setItems(items, action).create()
    }
    ...
    class Builder @JvmOverloads constructor(
         ...
         * @return the current [Builder] instance
         */
        fun setAction(entry: StreamDialogDefaultEntry, action: StreamDialogEntryAction): Builder {
        ...
        fun addMarkAsWatchedEntryIfNeeded(): Builder {
            ...
            if (isWatchHistoryEnabled && !isLiveStream(infoItem.streamType)) addEntry(StreamDialogDefaultEntry.MARK_AS_WATCHED)
            return this
        }
        ...
        fun addPlayWithKodiEntryIfNeeded(): Builder {
            if (shouldShowPlayWithKodi(context, infoItem.serviceId)) addEntry(StreamDialogDefaultEntry.PLAY_WITH_KODI)
            return this
        }
        ...
        fun create(): InfoItemDialog {
            if (addDefaultEntriesAutomatically) addDefaultEndEntries()
        ...
        companion object {
            ...
            fun reportErrorDuringInitialization(throwable: Throwable?, item: InfoItem) {
                showSnackbar(App.getApp().baseContext, ErrorInfo(throwable!!, UserAction.OPEN_INFO_ITEM_DIALOG, "none", item.serviceId))
            }
        }
    }
