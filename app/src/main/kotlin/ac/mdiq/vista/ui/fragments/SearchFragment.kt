...
import ac.mdiq.vista.databinding.FragmentSearchBinding
import ac.mdiq.vista.databinding.ItemSearchSuggestionBinding
...
import ac.mdiq.vista.extractor.search.SearchExtractor.NothingFoundException
...
import ac.mdiq.vista.settings.VistaSettings.showLocalSearchSuggestions
import ac.mdiq.vista.settings.VistaSettings.showRemoteSearchSuggestions
import ac.mdiq.vista.ui.util.BackPressable
import ac.mdiq.vista.ui.util.NavigationHelper.getIntentByLink
...
import ac.mdiq.vista.util.ExtractorHelper.getMoreSearchItems
import ac.mdiq.vista.util.ExtractorHelper.searchFor
import ac.mdiq.vista.util.ExtractorHelper.showMetaInfoInTextView
import ac.mdiq.vista.util.ExtractorHelper.suggestionsFor
...
import ac.mdiq.vista.util.ServiceHelper.getTranslatedFilterString
import ac.mdiq.vista.util.isInterruptedCaused
import java.util.*
...
class SearchFragment : BaseListFragment<SearchInfo, InfoItemsPage<InfoItem>>(), BackPressable {
    ...
    private fun updateService() {
        try {
            service = Vista.getService(serviceId)
        } catch (e: Exception) {
    ...
    override fun onResume() {
        ...
        showMetaInfoInTextView(if (metaInfo == null) null else listOf(*metaInfo!!),
            searchBinding!!.searchMetaInfoTextView, searchBinding!!.searchMetaInfoSeparator, disposables)

        if (searchString.isNullOrEmpty() || wasSearchFocused) {
            showKeyboardSearch()
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        if (requestCode == ReCaptchaActivity.RECAPTCHA_REQUEST) {
            if (resultCode == Activity.RESULT_OK && !searchString.isNullOrEmpty()) search(searchString, contentFilter, sortFilter)
    ...
    override fun reloadContent() {
        if (!searchString.isNullOrEmpty() || (searchEditText != null && searchEditText!!.text.isNotEmpty()))
            search(if (!searchString.isNullOrEmpty()) searchString else searchEditText!!.text.toString(), this.contentFilter, "")
        else {
    ...
    override fun onCreateOptionsMenu(menu: Menu, inflater: MenuInflater) {
        ...
        for (filter in service!!.getSearchQHFactory().availableContentFilter) {
            when (filter) {
                YoutubeSearchQueryHandlerFactory.MUSIC_SONGS -> {
                    val musicItem = menu.add(2, itemId++, 0, "YouTube Music")
                    musicItem.setEnabled(false)
                }
                PeertubeSearchQueryHandlerFactory.SEPIA_VIDEOS -> {
                    val sepiaItem = menu.add(2, itemId++, 0, "Sepia Search")
                    sepiaItem.setEnabled(false)
                }
            }
            menuItemToFilterName.put(itemId, filter)
            val item = menu.add(1, itemId++, 0, getTranslatedFilterString(filter, c!!))
            if (isFirstItem) {
    ...
    private fun showSearchOnStart() {
        Logd(TAG, "showSearchOnStart() called, searchQuery → $searchString, lastSearchedQuery → $lastSearchedString")
    ...
    private fun initSearchListeners() {
        ...
        searchEditText!!.setOnClickListener { v: View ->
            ...
            if (isTv(requireContext())) showKeyboardSearch()
        }
        ...
        suggestionListAdapter!!.setListener(object : SuggestionListAdapter.OnSuggestionItemSelected {
            ...
            override fun onSuggestionItemLongClick(item: SuggestionItem?) {
                if (item?.fromHistory == true) showDeleteSuggestionDialog(item)
            }
        })
        ...
        AlertDialog.Builder(requireActivity())
            ...
            .setPositiveButton(R.string.delete) { _: DialogInterface?, _: Int ->
                val onDelete = historyRecordManager!!.deleteSearchHistory(query)
                    ...
                    .subscribe({ _: Int? -> suggestionPublisher.onNext(searchEditText!!.text.toString()) },
                        { throwable: Throwable? -> showSnackBarError(ErrorInfo(throwable!!, UserAction.DELETE_FROM_HISTORY, "Deleting item failed")) })
    ...
    override fun onBackPressed(): Boolean {
        if (suggestionsPanelVisible && infoListAdapter!!.itemsList.isNotEmpty() && !isLoading.get()) {
            hideSuggestionsPanel()
            hideKeyboardSearch()
    ...
    private fun getLocalSuggestionsObservable(query: String?, similarQueryLimit: Int): Observable<MutableList<SuggestionItem>> {
        return historyRecordManager!!
            ...
            .toObservable()
            .map { searchHistoryEntries: List<String> ->
                ...
                when {
                    showLocalSuggestions && shallShowRemoteSuggestionsNow -> {
                        return@switchMap Observable.zip<MutableList<SuggestionItem>, MutableList<SuggestionItem>, MutableList<SuggestionItem>>(
                            getLocalSuggestionsObservable(query, 3), getRemoteSuggestionsObservable(query))
                        { local: MutableList<SuggestionItem>, remote: MutableList<SuggestionItem> ->
                            remote.removeIf { remoteItem: SuggestionItem -> local.stream().anyMatch { localItem: SuggestionItem -> localItem == remoteItem } }
                            local.addAll(remote)
                            local }
                            .materialize()
                    }
                    showLocalSuggestions -> return@switchMap getLocalSuggestionsObservable(query, 25).materialize()
                    shallShowRemoteSuggestionsNow -> return@switchMap getRemoteSuggestionsObservable(query).materialize()
                    else -> return@switchMap Single.fromCallable<MutableList<SuggestionItem>> { mutableListOf() }.toObservable().materialize()
                }
            }
            .subscribeOn(Schedulers.io())
            .observeOn(AndroidSchedulers.mainThread())
            .subscribe({ listNotification: Notification<MutableList<SuggestionItem>> ->
                if (listNotification.isOnNext) {
                    if (listNotification.value != null) handleSuggestions(listNotification.value!!)
                } else if (listNotification.isOnError && listNotification.error != null && !listNotification.error!!.isInterruptedCaused)
                    showSnackBarError(ErrorInfo(listNotification.error!!, UserAction.GET_SUGGESTIONS, searchString!!, serviceId))
            }, { throwable: Throwable? -> showSnackBarError(ErrorInfo(throwable!!, UserAction.GET_SUGGESTIONS, searchString!!, serviceId)) })
    }
    ...
    private fun search(theSearchString: String?, theContentFilter: Array<String?>, theSortFilter: String?) {
        Logd(TAG, "search() called with: query = [$theSearchString]")
        if (theSearchString.isNullOrEmpty()) return
        try {
            val streamingService = Vista.getServiceByUrl(theSearchString)
            showLoading()
            disposables.add(Observable
                .fromCallable { getIntentByLink(requireActivity(), streamingService, theSearchString) }
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
        ...
        disposables.add(historyRecordManager!!.onSearched(serviceId, theSearchString)
            .observeOn(AndroidSchedulers.mainThread())
            .subscribe({ }, { throwable: Throwable? -> showSnackBarError(ErrorInfo(throwable!!, UserAction.SEARCHED, theSearchString, serviceId)) }))
    ...
    public override fun startLoading(forceLoad: Boolean) {
        ...
        searchDisposable = searchFor(serviceId, searchString, listOf(*contentFilter), sortFilter)
            .subscribeOn(Schedulers.io())
            .observeOn(AndroidSchedulers.mainThread())
            .doOnEvent { _: SearchInfo?, _: Throwable? -> isLoading.set(false) }
            .subscribe({ result: SearchInfo -> this.handleResult(result) }, { exception: Throwable -> this.onItemError(exception) })
    }
    ...
    override fun loadMoreItems() {
        ...
        searchDisposable = getMoreSearchItems(serviceId, searchString, listOf(*contentFilter), sortFilter, nextPage)
            .subscribeOn(Schedulers.io())
            .observeOn(AndroidSchedulers.mainThread())
            .doOnEvent { _: InfoItemsPage<InfoItem>?, _: Throwable? -> isLoading.set(false) }
            .subscribe({ result: InfoItemsPage<InfoItem> -> this.handleNextItems(result)}, { exception: Throwable -> this.onItemError(exception) })
    }
    ...
    private fun onItemError(exception: Throwable) {
        if (exception is NothingFoundException) {
            infoListAdapter!!.clearStreamItemList()
            showEmptyState()
        } else showError(ErrorInfo(exception, UserAction.SEARCHED, searchString!!, serviceId))
    }
    ...
    private fun changeContentFilter(item: MenuItem, theContentFilter: List<String?>) {
        ...
        if (!searchString.isNullOrEmpty()) search(searchString, contentFilter, sortFilter)
    }

    private fun setQuery(theServiceId: Int, theSearchString: String, theContentFilter: Array<String?>, theSortFilter: String) {
    ...
    override fun handleResult(result: SearchInfo) {
        ...
        if (exceptions.isNotEmpty() && !(exceptions.size == 1 && exceptions[0] is NothingFoundException))
        ...
        metaInfo = result.metaInfo.filterNotNull().toTypedArray()
        showMetaInfoInTextView(result.metaInfo.filterNotNull(), searchBinding!!.searchMetaInfoTextView, searchBinding!!.searchMetaInfoSeparator, disposables)
        ...
        if (infoListAdapter!!.itemsList.isEmpty()) {
            if (result.relatedItems.isNotEmpty()) infoListAdapter!!.addInfoItemList(result.relatedItems)
            else {
                infoListAdapter!!.clearStreamItemList()
                showEmptyState()
                return
    ...
    private fun handleSearchSuggestion() {
        if (searchSuggestion.isNullOrEmpty()) searchBinding!!.correctSuggestion.visibility = View.GONE
        else {
            ...
            searchBinding!!.correctSuggestion.setOnClickListener {
                searchBinding!!.correctSuggestion.visibility = View.GONE
                search(searchSuggestion, contentFilter, sortFilter)
                searchEditText!!.setText(searchSuggestion)
            }
            searchBinding!!.correctSuggestion.setOnLongClickListener {
                searchEditText!!.setText(searchSuggestion)
    ...
    override fun handleNextItems(result: InfoItemsPage<InfoItem>) {
        ...
        if (result.errors.isNotEmpty()) showSnackBarError(ErrorInfo(result.errors, UserAction.SEARCHED,
            "\"$searchString\" → pageUrl: ${nextPage!!.url}, pageIds: ${nextPage!!.url}, pageCookies: ${nextPage!!.cookies}", serviceId))
    ...
    fun getSuggestionMovementFlags(viewHolder: RecyclerView.ViewHolder): Int {
        ...
        return if (item.fromHistory) ItemTouchHelper.Callback.makeMovementFlags(0, ItemTouchHelper.LEFT or ItemTouchHelper.RIGHT) else 0
    }
    ...
    class SuggestionItem(
            @JvmField val fromHistory: Boolean,
            @JvmField val query: String) {

        override fun equals(other: Any?): Boolean {
            if (other is SuggestionItem) return query == other.query
            return false
        }
    ...
    class SuggestionListAdapter : ListAdapter<SuggestionItem, SuggestionListAdapter.SuggestionItemHolder>(SuggestionItemCallback()) {
        ...
        override fun onCreateViewHolder(parent: ViewGroup, viewType: Int) : SuggestionItemHolder {
            return SuggestionItemHolder(ItemSearchSuggestionBinding.inflate(LayoutInflater.from(parent.context), parent, false))
        }
        override fun onBindViewHolder(holder: SuggestionItemHolder, position: Int) {
            val currentItem = getItem(position)
            holder.updateFrom(currentItem)
            holder.itemBinding.suggestionSearch.setOnClickListener { listener?.onSuggestionItemSelected(currentItem) }
            holder.itemBinding.suggestionSearch.setOnLongClickListener {
                listener?.onSuggestionItemLongClick(currentItem)
                true
            }
            holder.itemBinding.suggestionInsert.setOnClickListener { listener?.onSuggestionItemInserted(currentItem) }
        }
        ...
        class SuggestionItemHolder internal constructor(val itemBinding: ItemSearchSuggestionBinding) : RecyclerView.ViewHolder(itemBinding.root) {
    ...
    companion object {
        ...
        fun getInstance(serviceId: Int, searchString: String): SearchFragment {
            ...
            Logd("SearchFragment", "getInstance serviceId: $serviceId searchString: $searchString")
            searchFragment.setQuery(serviceId, searchString, arrayOfNulls(0), "")
            if (searchString.isNotEmpty()) searchFragment.setSearchOnResume()
