...
import ac.mdiq.vista.ui.views.VistaRecyclerView
...
abstract class BaseListInfoFragment<I : InfoItem, L : ListInfo<I>> protected constructor(private val errorUserAction: UserAction)
     ...
     * You can use the default implementations from [ac.mdiq.vista.util.ExtractorHelper].
     *
    ...
    public override fun startLoading(forceLoad: Boolean) {
        ...
        currentWorker = loadResult(forceLoad)
            ...
            .subscribe({ result: L ->
                ...
                handleResult(result)
            }, { throwable: Throwable? -> showError(ErrorInfo(throwable!!, errorUserAction, "Start loading: $url", serviceId)) })
    }

    /**
     * Implement the logic to load more items.
     * You can use the default implementations
     * from [ac.mdiq.vista.util.ExtractorHelper].
    ...
    private fun forbidDownwardFocusScroll() {
        if (itemsList is VistaRecyclerView) (itemsList as VistaRecyclerView).setFocusScrollAllowed(false)
    }

    private fun allowDownwardFocusScroll() {
        if (itemsList is VistaRecyclerView) (itemsList as VistaRecyclerView).setFocusScrollAllowed(true)
    }
