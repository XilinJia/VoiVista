...
import ac.mdiq.vista.local.subscription.item.ChannelItem
import ac.mdiq.vista.local.subscription.item.FeedGroupCardGridItem
import ac.mdiq.vista.local.subscription.item.FeedGroupCardItem
...
class SubscriptionViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private var feedGroupItemsDisposable = Flowable
        ...
        .map { (feedGroups, listViewMode) -> Pair(feedGroups.map(if (listViewMode) ::FeedGroupCardItem else ::FeedGroupCardGridItem), listViewMode) }
        .subscribeOn(Schedulers.io())
        .subscribe({ mutableFeedGroupsLiveData.postValue(it) }, { mutableStateLiveData.postValue(SubscriptionState.ErrorState(it)) }, )
    ...
    private var stateItemsDisposable = subscriptionManager.subscriptions()
        ...
        .subscribeOn(Schedulers.io())
        .subscribe({ mutableStateLiveData.postValue(SubscriptionState.LoadedState(it)) }, { mutableStateLiveData.postValue(SubscriptionState.ErrorState(it)) })
