...
import ac.mdiq.vista.player.MediaItemTag
...
import ac.mdiq.vista.ui.views.player.PlayerFastSeekOverlay.PerformListener
import ac.mdiq.vista.ui.views.player.PlayerFastSeekOverlay.PerformListener.FastSeekDirection
...
import ac.mdiq.vista.util.image.PicassoHelper.loadSeekbarThumbnailPreview
...
abstract class VideoPlayerUi protected constructor(playerManager: PlayerManager, playerBinding: PlayerBinding) :
    ...
    private var surfaceIsSetup: Boolean = false
    ...
    protected open fun initListeners() {
        ...
        onLayoutChangeListener =
            OnLayoutChangeListener { v: View, _: Int, _: Int, _: Int, _: Int, _: Int, _: Int, _: Int, _: Int ->
    ...
    private fun deinitPlayerSeekOverlay() {
        binding.fastSeekOverlay.seekSecondsSupplier(null).performListener(null)
    }
    ...
    override fun onBroadcastReceived(intent: Intent?) {
        ...
        if (Intent.ACTION_CONFIGURATION_CHANGED == intent!!.action) updateEndScreenThumbnail(playerManager.thumbnail)
    }
    ...
    private fun updateEndScreenThumbnail(thumbnail: Bitmap?) {
        ...
        Logd(TAG, "Thumbnail - onThumbnailLoaded() called with: currentThumbnail = [$thumbnail], ${thumbnail.width}x${thumbnail.height}, scaled end screen height = $endScreenHeight, scaled end screen width = ${endScreenBitmap.width}")
...
//            Logd(TAG, "notifyProgressUpdateToListeners() called with: isVisible = $isControlsVisible, currentProgress = [$currentProgress], duration = [$duration], bufferPercent = [$bufferPercent]")
    ...
    override fun onProgressChanged(seekBar: SeekBar, progress: Int, fromUser: Boolean) {
        ...
        SeekbarPreviewThumbnailHelper.tryResizeAndSetSeekbarPreviewThumbnail(playerManager.context,
    ...
    private fun showHideShadow(show: Boolean, duration: Long) {
    ...
    override fun onRepeatModeChanged(repeatMode: @Player.RepeatMode Int) {
        ...
        when (repeatMode) {
            Player.REPEAT_MODE_ALL -> binding.repeatButton.setImageResource(R.drawable.exo_styled_controls_repeat_all)
            Player.REPEAT_MODE_ONE -> binding.repeatButton.setImageResource(R.drawable.exo_styled_controls_repeat_one)
            /* repeatMode == REPEAT_MODE_OFF */
            else ->  binding.repeatButton.setImageResource(R.drawable.exo_styled_controls_repeat_off)
        }
    }
    ...
    private fun buildQualityMenu() {
        ...
        val availableStreams = Optional.ofNullable<MediaItemTag>(playerManager.currentMetadata)
            ...
            .map { it.sortedVideoStreams }
            .orElse(null)
        if (availableStreams.isNullOrEmpty()) return
        ...
        playerManager.selectedVideoStream.ifPresent { s: VideoStream -> binding.qualityTextView.text = s.resolution }
    }
    ...
    private fun buildAudioTrackMenu() {
        ...
        playerManager.selectedAudioStream?.map { s: AudioStream? -> binding.audioTrackTextView.text = audioTrackName(context, s!!) }
    ...
    private fun buildPlaybackSpeedMenu() {
        ...
        for (i in PLAYBACK_SPEEDS.indices) {
            playbackSpeedPopupMenu!!.menu.add(POPUP_MENU_ID_PLAYBACK_SPEED, i, Menu.NONE, PlayerHelper.formatSpeed(PLAYBACK_SPEEDS[i].toDouble()))
        }
        binding.playbackSpeed.text = PlayerHelper.formatSpeed(playerManager.playbackSpeed.toDouble())
    ...
    private fun onQualityClicked() {
        ...
        playerManager.selectedVideoStream
            .map { s: VideoStream -> MediaFormat.getNameById(s.getFormatId()) + " " + s.resolution }
    ...
    override fun onMenuItemClick(menuItem: MenuItem): Boolean {
        Logd(TAG, "onMenuItemClick() called with: menuItem = [$menuItem], menuItem.getItemId = [${menuItem.itemId}]")
    ...
    override fun onTextTracksChanged(currentTracks: Tracks) {
        ...
        val selectedTracks = textTracks.stream()
            ...
            .map { info: Tracks.Group -> info.mediaTrackGroup.getFormat(0) }
            .findFirst()
        ...
        when (keyCode) {
            ...
            KeyEvent.KEYCODE_DPAD_UP, KeyEvent.KEYCODE_DPAD_LEFT, KeyEvent.KEYCODE_DPAD_DOWN, KeyEvent.KEYCODE_DPAD_RIGHT, KeyEvent.KEYCODE_DPAD_CENTER -> {
                if ((binding.root.hasFocus() && !binding.playbackControlRoot.hasFocus()) || this.isAnyListViewOpen) {
                    ...
                    if (isControlsVisible) hideControls(DEFAULT_CONTROLS_DURATION, DPAD_CONTROLS_HIDE_TIME)
                    else {
                        binding.playPauseButton.requestFocus()
                        showControlsThenHide()
                        showSystemUIPartially()
    ...
    fun setupVideoSurfaceIfNeeded() {
        if (!surfaceIsSetup && playerManager.exoPlayer != null && binding.root.parent != null) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) { // >=API23
                ...
                if (binding.surfaceView.holder.surface.isValid) playerManager.exoPlayer!!.setVideoSurfaceHolder(binding.surfaceView.holder)
            } else playerManager.exoPlayer!!.setVideoSurfaceView(binding.surfaceView)
...
//        }
    ...
    class SeekbarPreviewThumbnailHolder {
        ...
        private fun getFrameSetForType(framesets: List<Frameset?>, seekbarPreviewType: Int): Frameset? {
        ...
        private fun generateDataFrom(frameset: Frameset, updateRequestIdentifier: UUID) {
            ...
            if (sw != null) Logd(TAG, "Generation of seekbarPreviewData took " + sw.stop())
        }
        ...
        private fun getBitMapFrom(url: String?): Bitmap? {
            ...
            try {
                ...
                if (sw != null) Logd(TAG, "Download of bitmap for seekbarPreview from '$url' took ${sw.stop()}")
                return bitmap
            } catch (ex: Exception) {
    ...
    object SeekbarPreviewThumbnailHelper {
        ...
        fun getSeekbarPreviewThumbnailType(context: Context): Int {
            val type = PreferenceManager.getDefaultSharedPreferences(context).getString(context.getString(R.string.seekbar_preview_thumbnail_key), "")
            return when (type) {
                context.getString(R.string.seekbar_preview_thumbnail_none) -> SeekbarPreviewThumbnailType.NONE
                context.getString(R.string.seekbar_preview_thumbnail_low_quality) -> SeekbarPreviewThumbnailType.LOW_QUALITY
                else -> SeekbarPreviewThumbnailType.HIGH_QUALITY // default
            }
        }
        fun tryResizeAndSetSeekbarPreviewThumbnail(context: Context, previewThumbnail: Bitmap?, currentSeekbarPreviewThumbnail: ImageView,
