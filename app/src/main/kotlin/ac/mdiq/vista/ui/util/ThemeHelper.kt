 ...
 * ThemeHelper.kt is part of Vista
 *
 * License: GPL-3.0+
...
object ThemeHelper {
    /**
     * Apply the selected theme (on Vista settings) in the context
    ...
    /**
     * Return true if the selected theme (on Vista settings) is the Light theme.
     *
    ...
    fun getThemeForService(context: Context, serviceId: Int): Int {
        ...
        when (selectedThemeKey) {
            lightThemeKey -> baseTheme = R.style.LightTheme
            blackThemeKey -> baseTheme = R.style.BlackTheme
            automaticDeviceThemeKey -> {
                if (isDeviceDarkThemeEnabled(context)) {
                    // use the dark theme variant preferred by the user
                    val selectedNightThemeKey = getSelectedNightThemeKey(context)
                    baseTheme = if (selectedNightThemeKey == blackThemeKey) R.style.BlackTheme else R.style.DarkTheme
                } else {
        ...
        if (serviceId <= -1) return baseTheme
    ...
    fun getSettingsThemeStyle(context: Context): Int {
        ...
        when (selectedTheme) {
            lightTheme -> return R.style.LightSettingsTheme
            blackTheme -> return R.style.BlackSettingsTheme
            automaticDeviceTheme -> {
                if (isDeviceDarkThemeEnabled(context)) {
                    // use the dark theme variant preferred by the user
                    val selectedNightTheme = getSelectedNightThemeKey(context)
                    return when (selectedNightTheme) {
                        blackTheme -> R.style.BlackSettingsTheme
                        else -> R.style.DarkSettingsTheme
                    }
                } else return R.style.LightSettingsTheme   // there is only one day theme
            }
            else -> return R.style.DarkSettingsTheme    // default to dark theme
        }
    }
    ...
    private fun getSelectedNightThemeKey(context: Context): String? {
        ...
        return PreferenceManager.getDefaultSharedPreferences(context).getString(nightThemeKey, defaultNightTheme)
    }
     ...
     */
    private fun isDeviceDarkThemeEnabled(context: Context): Boolean {
    ...
    fun setDayNightMode(context: Context, selectedThemeKey: String?) {
        ...
        when (selectedThemeKey) {
            ...
            res.getString(R.string.dark_theme_key), res.getString(R.string.black_theme_key) -> AppCompatDelegate.setDefaultNightMode(AppCompatDelegate.MODE_NIGHT_YES)
    ...
    fun getItemViewMode(context: Context): ItemViewMode {
        ...
        when (listMode) {
            context.getString(R.string.list_view_mode_list_key) -> result = ItemViewMode.LIST
            context.getString(R.string.list_view_mode_grid_key) -> result = ItemViewMode.GRID
            context.getString(R.string.list_view_mode_card_key) -> result = ItemViewMode.CARD
            else -> {
                ...
                result = if (useGrid) ItemViewMode.GRID else ItemViewMode.LIST
            }
        }
        return result
    }
    ...
    fun getGridSpanCountStreams(context: Context): Int {
        val res = context.resources
        return getGridSpanCount(context, res.getDimensionPixelSize(R.dimen.video_item_grid_thumbnail_image_width) + res.getDimensionPixelSize(R.dimen.video_item_search_padding) * 2)
    }
