...
import ac.mdiq.vista.databinding.DownloadLoadingDialogBinding
...
import ac.mdiq.vista.util.isNetworkRelated
...
import ac.mdiq.vista.ui.util.NavigationHelper.openSearch
import ac.mdiq.vista.ui.util.NavigationHelper.playOnBackgroundPlayer
import ac.mdiq.vista.ui.util.NavigationHelper.playOnExternalAudioPlayer
import ac.mdiq.vista.ui.util.NavigationHelper.playOnExternalVideoPlayer
...
import ac.mdiq.vista.util.UrlFinder.Companion.firstUrlFromInput
...
@UnstableApi class RouterActivity : AppCompatActivity() {
    ...
    override fun onStart() {
        ...
        if (supportFragmentManager.fragments.isEmpty()) handleUrl(currentUrl ?: "")
    }
    ...
    private fun handleUrl(url: String) {
        disposables.add(Observable
            .fromCallable {
                try {
                    if (currentServiceId == -1) {
                        currentService = Vista.getServiceByUrl(url)
                        ...
                        currentService = Vista.getService(currentServiceId)
                    }
            ...
            .subscribe({ isUrlSupported: Boolean ->
                ...
                handleError(this, ErrorInfo(throwable!!, UserAction.SHARE_TO_VOIVISTA, "Getting service from url: $url"))
            }))
    }
    ...
    protected fun onSuccess() {
        ...
        if (choiceChecker.isAvailableAndSelected(R.string.show_info_key,
        ...
        if (choiceChecker.isAvailableAndSelected(R.string.video_player_key,
            ...
            val isVideoPlayerSelected = selectedChoice == getString(R.string.video_player_key) || selectedChoice == getString(
                R.string.popup_player_key)
    ...
    private fun showDialog(choices: List<AdapterChoiceItem>) {
        ...
        if (selectedRadioPosition == -1) {
            ...
            if (!lastSelectedPlayer.isNullOrEmpty()) {
        ...
        if (selectedChoiceKey == getString(R.string.show_info_key) || canHandleChoiceLikeShowInfo(selectedChoiceKey)) {
            disposables.add(Observable
                ...
                .subscribe({ intent: Intent? ->
                    ...
                    handleError(this, ErrorInfo(throwable!!, UserAction.SHARE_TO_VOIVISTA, "Starting info activity: $currentUrl"))
                })
            )
            return
        }
                ...
                when (info) {
                    ...
                    is ChannelInfo -> {
                        ...
                        if (playableTab.isPresent) {
                            playQueue = ChannelTabPlayQueue(info.serviceId, playableTab.get())
                        } else {
