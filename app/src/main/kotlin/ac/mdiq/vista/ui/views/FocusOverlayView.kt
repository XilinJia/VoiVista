 ...
 * FocusOverlayView.kt is part of Vista
 *
 * License: GPL-3.0+
...
class FocusOverlayView(context: Context)
    : Drawable(), OnGlobalFocusChangeListener, OnDrawListener, OnGlobalLayoutListener, OnScrollChangedListener, OnTouchModeChangeListener {
    ...
    override fun onGlobalFocusChanged(oldFocus: View?, newFocus: View?) {
        focused = if (newFocus != null) WeakReference(newFocus) else null
        updateRect()
    ...
    private fun updateRect() {
        ...
        if (focusedView != null && isShown(focusedView)) focusedView.getGlobalVisibleRect(focusRect)
        if (shouldClearFocusRect(focusedView, focusRect)) focusRect.setEmpty()
        if (l != focusRect.left || r != focusRect.right || t != focusRect.top || b != focusRect.bottom) invalidateSelf()
    }
    ...
    override fun draw(canvas: Canvas) {
        if (!isInTouchMode && focusRect.width() != 0) canvas.drawRect(focusRect, rectPaint)
    }
...
//            TODO: WindowCallbackWrapper can only be called from within the same library group prefix (referenced groupId=`androidx.appcompat` with prefix androidx from groupId=`Vista`)
...
//            }
        ...
        private fun clearFocusObstacles(viewGroup: ViewGroup) {
            ...
            for (i in 0 until childCount) {
                ...
                if (view is ViewGroup) clearFocusObstacles(view)
            }
        }
    }
}
