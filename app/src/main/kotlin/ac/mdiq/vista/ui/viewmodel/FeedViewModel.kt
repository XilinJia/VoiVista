package ac.mdiq.vista.ui.viewmodel
...
import ac.mdiq.vista.manager.FeedDatabaseManager
...
import ac.mdiq.vista.local.feed.service.FeedEventManager.Event.ErrorResultEvent
import ac.mdiq.vista.local.feed.service.FeedEventManager.Event.IdleEvent
import ac.mdiq.vista.local.feed.service.FeedEventManager.Event.ProgressEvent
import ac.mdiq.vista.local.feed.service.FeedEventManager.Event.SuccessResultEvent
import ac.mdiq.vista.util.DEFAULT_THROTTLE_TIMEOUT
...
class FeedViewModel(
        ...
        initialShowFutureItems: Boolean) : ViewModel() {
    ...
    private var combineDisposable = Flowable
        .combineLatest(FeedEventManager.events(),
            showPlayedItemsFlowable,
            showPartiallyPlayedItemsFlowable,
            showFutureItemsFlowable,
            feedDatabaseManager.notLoadedCount(groupId),
            feedDatabaseManager.oldestSubscriptionUpdate(groupId),
            Function6 { t1: FeedEventManager.Event, t2: Boolean, t3: Boolean, t4: Boolean, t5: Long, t6: List<OffsetDateTime> ->
            ...
            .map { (event, showPlayedItems, showPartiallyPlayedItems, showFutureItems, notLoadedCount, oldestUpdate) ->
                val streamItems = if (event is SuccessResultEvent || event is IdleEvent)
                    feedDatabaseManager.getStreams(groupId, showPlayedItems, showPartiallyPlayedItems, showFutureItems).blockingGet(arrayListOf())
                else arrayListOf()
                CombineResultDataHolder(event, streamItems, notLoadedCount, oldestUpdate) }
            .observeOn(AndroidSchedulers.mainThread())
            .subscribe { (event, listFromDB, notLoadedCount, oldestUpdate) ->
                Logd(TAG, "combineDisposable subscribe event: $event")
