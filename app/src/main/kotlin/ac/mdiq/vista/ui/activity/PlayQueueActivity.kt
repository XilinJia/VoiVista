...
import ac.mdiq.vista.databinding.ActivityPlayerQueueControlBinding
...
import ac.mdiq.vista.ui.player.PlayerEventListener
...
import ac.mdiq.vista.player.playqueue.*
import ac.mdiq.vista.ui.dialog.PlaylistDialog.Companion.showForPlayQueue
import ac.mdiq.vista.ui.player.PlayQueueAdapter
import ac.mdiq.vista.ui.player.PlayQueueItemHolder
import ac.mdiq.vista.ui.player.PlayQueueItemTouchCallback
...
@UnstableApi class PlayQueueActivity : AppCompatActivity(), PlayerEventListener, OnSeekBarChangeListener, View.OnClickListener,
    ...
    override fun onCreateOptionsMenu(m: Menu): Boolean {
        ...
        if (playerManager != null) onPlaybackParameterChanged(playerManager!!.playbackParameters)
        return true
    }
    ...
    override fun onPrepareOptionsMenu(m: Menu): Boolean {
        if (playerManager != null) {
            menu!!.findItem(R.id.action_switch_popup).setVisible(!playerManager!!.popupPlayerSelected())
            menu!!.findItem(R.id.action_switch_background).setVisible(!playerManager!!.audioPlayerSelected())
        }
    ...
    private fun getServiceConnection(): ServiceConnection {
        return object : ServiceConnection {
            ...
            override fun onServiceConnected(name: ComponentName, service: IBinder) {
                ...
                if (playerManager == null || playerManager!!.playQueue == null || playerManager!!.exoPlayerIsNull()) unbind()
                else {
    ...
    private val itemTouchCallback: ItemTouchHelper.SimpleCallback
        get() = object : PlayQueueItemTouchCallback() {
            ...
            override fun onSwiped(index: Int) {
                if (index != -1) playerManager!!.playQueue!!.remove(index)
            }
        }
        ...
        get() = object : PlayQueueItemBuilder.OnSelectedListener {
            ...
            override fun held(item: PlayQueueItem, view: View) {
                if (playerManager != null && playerManager!!.playQueue!!.indexOf(item) != -1)
                    openPopupMenu(playerManager!!.playQueue!!, item, view, false, supportFragmentManager, this@PlayQueueActivity)
            }
    ...
    private fun scrollToSelected() {
        ...
        if (queueControlBinding!!.playQueue.layoutManager is LinearLayoutManager) {
            val layout = queueControlBinding!!.playQueue.layoutManager as LinearLayoutManager?
            currentVisibleIndex = layout!!.findFirstVisibleItemPosition()
        } else currentVisibleIndex = 0

        val distance = abs((currentPlayingIndex - currentVisibleIndex).toDouble()).toInt()
        if (distance < SMOOTH_SCROLL_MAXIMUM_DISTANCE) queueControlBinding!!.playQueue.smoothScrollToPosition(currentPlayingIndex)
    ...
    override fun onClick(view: View) {
        ...
        when (view.id) {
            queueControlBinding!!.controlRepeat.id -> playerManager!!.cycleNextRepeatMode()
            queueControlBinding!!.controlBackward.id -> playerManager!!.playPrevious()
            queueControlBinding!!.controlFastRewind.id -> playerManager!!.fastRewind()
            queueControlBinding!!.controlPlayPause.id -> playerManager!!.playPause()
            queueControlBinding!!.controlFastForward.id -> playerManager!!.fastForward()
            queueControlBinding!!.controlForward.id -> playerManager!!.playNext()
            queueControlBinding!!.controlShuffle.id -> playerManager!!.toggleShuffleModeEnabled()
            queueControlBinding!!.metadata.id -> scrollToSelected()
            queueControlBinding!!.liveSync.id -> playerManager!!.seekToDefault()
        }
    }
    ...
    override fun onStopTrackingTouch(seekBar: SeekBar) {
        if (playerManager != null) playerManager!!.seekTo(seekBar.progress.toLong())
        queueControlBinding!!.seekDisplay.visibility = View.GONE
        seeking = false
    }

    override fun onQueueUpdate(queue: PlayQueue?) {
        if (queue == null) queueControlBinding!!.playQueue.adapter = null
        else {
    ...
    override fun onProgressUpdate(currentProgress: Int, duration: Int, bufferPercent: Int) {
        // Set buffer progress
        queueControlBinding!!.seekBar.secondaryProgress = (queueControlBinding!!.seekBar.max * (bufferPercent.toFloat() / 100)).toInt()
        ...
        if (playerManager != null) queueControlBinding!!.liveSync.isClickable = !playerManager!!.isLiveEdge
    ...
    private fun onPlayModeChanged(repeatMode: Int, shuffled: Boolean) {
        when (repeatMode) {
            androidx.media3.common.Player.REPEAT_MODE_OFF ->
                queueControlBinding!!.controlRepeat.setImageResource(R.drawable.exo_styled_controls_repeat_off)
            androidx.media3.common.Player.REPEAT_MODE_ONE ->
                queueControlBinding!!.controlRepeat.setImageResource(R.drawable.exo_styled_controls_repeat_one)
            androidx.media3.common.Player.REPEAT_MODE_ALL ->
                queueControlBinding!!.controlRepeat.setImageResource(R.drawable.exo_styled_controls_repeat_all)
        }
    ...
    private fun buildAudioTrackMenu() {
        ...
        if (availableStreams == null || availableStreams.size < 2 || selectedAudioStream.isEmpty) audioTrackSelector.setVisible(false)
        else {
