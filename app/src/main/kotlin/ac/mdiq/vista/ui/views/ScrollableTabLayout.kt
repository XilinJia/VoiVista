...
class ScrollableTabLayout : TabLayout {
    ...
    override fun addTab(tab: Tab, position: Int, setSelected: Boolean) {
        ...
        if (tabMode != MODE_SCROLLABLE) remeasureTabs()
    }
    ...
    override fun removeTabAt(position: Int) {
        ...
        if (tabMode != MODE_FIXED) remeasureTabs()
    }
    ...
    override fun onVisibilityChanged(changedView: View, visibility: Int) {
        ...
        if (changedView === this) {
            if (prevVisibility == INVISIBLE) remeasureTabs()
    ...
    private fun hasMultipleTabs() {
        visibility = if (tabCount > 1) VISIBLE else GONE
    }
    ...
    private fun remeasureTabs() {
        ...
        for (i in 0 until count) {
            ...
            if (child.visibility == VISIBLE) cwDouble += max(child.minimumWidth.toDouble(), child.measuredWidth.toDouble())
        }
        contentWidth = cwDouble.toInt()
        if (contentWidth > layoutWidth) this.mode = MODE_SCROLLABLE
        else this.mode = MODE_FIXED
    }
