 ...
 */
class TextEllipsizer(private val view: TextView, private val maxLines: Int, private var streamingService: StreamingService?) {
    ...
    private fun expand() {
        view.maxLines = EXPANDED_LINES
        linkifyContentView { isEllipsized = false }
    }
    ...
    fun ellipsize() {
        ...
        linkifyContentView {
            ...
            if (charSeqText != null && view.lineCount > maxLines) {
                ...
                while (lineWidth - removedCharactersWidth + ellipsisWidthPx + 2.0f > layoutWidth && end >= lineStart) {
                    end -= 1
                    // recalculate each time to account for ligatures or other similar things
                    removedCharactersWidth = paintAtContentSize.measureText(text.substring(end, lineEnd))
                }
    ...
    private fun linkifyContentView(consumer: Consumer<View?>) {
        ...
        disposable.clear()
        fromDescription(view, content, HtmlCompat.FROM_HTML_MODE_LEGACY, streamingService, streamUrl, disposable) { v: TextView? ->
            consumer.accept(v)
    ...
    private fun notifyStateChangeListener(oldState: Boolean) {
        if (oldState != isEllipsized && stateChangeListener != null) stateChangeListener!!.accept(isEllipsized)
    }
